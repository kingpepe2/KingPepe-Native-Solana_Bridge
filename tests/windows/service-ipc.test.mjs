...
import { nativeFrostIpcHandler, ProtectedRemoteFrostPeer, openProtectedNativeSigner } from "../../native/frost/signer/protected-service.mjs";
import { WindowsProtectedFrostStateStore } from "../../native/frost/state/windows-protected-state-store.mjs";
import { NativeFrostSigner, NativeFrostCoordinator, createNativeSigningPolicy, createMainnetNativeSigningPolicy, createNativeFrostSigningRequest, runTwoPartyDkg,
...
import { deploymentFixture, mainnetDeploymentFixture } from "../integration/deployment-fixture.mjs";
...
import { nativeProgressFixture, mainnetNativeProgressFixture } from "../integration/native-progress-fixture.mjs";
...
import { ProtectedNativeSweepOutbox, initialNativeSweepOutbox } from "../../services/relayer/native-sweep-outbox.mjs";
...
function fixture(t, role = "KINGPEPE_FROST_A", clientRole = "COORDINATOR", mainnetDeployment) {
  ...
  if (mainnetDeployment) for (const opts of [serverOptions, clientOptions]) Object.assign(opts.context,
    { environment: "mainnet", nativeGenesis: mainnetDeployment.nativeGenesis, solanaDeployment: mainnetDeployment.solanaDeployment, keyEpoch: mainnetDeployment.keyEpoch });
  ...
  const open = store => mainnetDeployment ? ProtectedServiceIpc.fromMainnetStore(store, mainnetDeployment) : new ProtectedServiceIpc(store);
  const server = open(serverStore), client = open(clientStore);
...
function mainnetFixtureDeployment() {
  const keys = Object.fromEntries(["manager", "transceiver", "mint"].map(k => [k, h("isolated-mainnet-ipc-" + k)]));
  return { environment: "mainnet", protocolId: 1, nativeNetwork: NATIVE_MAINNET_DOMAIN, nativeGenesis: NATIVE_MAINNET_GENESIS,
    solanaGenesis: SOLANA_MAINNET_GENESIS, solanaDeployment: mainnetDeploymentIdentity(Object.fromEntries(
      Object.entries(keys).map(([k, v]) => [k, base58Encode(Buffer.from(v, "hex"))]))),
    managerProgramId: keys.manager, transceiverProgramId: keys.transceiver, mint: keys.mint, keyEpoch: 1 };
}

test("Mainnet protected transport requires complete deployment identity and never upgrades a legacy caller", async t => {
  const d = mainnetFixtureDeployment(), f = fixture(t, "KINGPEPE_FROST_A", "COORDINATOR", d);
  assert.throws(() => new ProtectedServiceIpc(f.clientStore), /IpcLocalEnrollmentRequired/);
  for (const mutation of [{ environment: "devnet" }, { nativeGenesis: REGTEST_GENESIS }, { nativeNetwork: 8000111 },
    { solanaGenesis: h("wrong-cluster") }, { mint: h("wrong-mint") }, { managerProgramId: h("wrong-program") },
    { transceiverProgramId: h("wrong-transceiver") }, { solanaDeployment: h("wrong-deployment") }, { keyEpoch: 2 }])
    assert.throws(() => ProtectedServiceIpc.fromMainnetStore(f.clientStore, { ...d, ...mutation }), /MainnetProtectedDeploymentRejected/);
  let calls = 0;
  const port = await f.server.listen(input => { calls++; assert.equal(input.method, "verifyNativeEvidence"); return { received: true }; });
  assert.deepEqual(await f.client.request(port, request()), { received: true });
  const localOptions = options(f.root, "same-cert-local-context", "COORDINATOR");
  const localStore = enroll(localOptions, f.clientCert, f.serverCert, "KINGPEPE_FROST_A"), local = new ProtectedServiceIpc(localStore);
  cleanups.get(f.root).push(() => { local.close(); localStore.close(); });
  // Even identical pinned TLS identities cannot cross the deployment/network framing.
  await assert.rejects(local.request(port, request({ operationId: h("cross-network") })));
  await assert.rejects(f.client.request(port, request({ method: "resume" })));
  assert.equal(calls, 1);
});

test("Mainnet protected supervisor starts paused, blocks signing and retains reconciliation faults on reopen", async t => {
  const d = mainnetFixtureDeployment(), root = temporary(t), opts = options(root, "mainnet-authority", "SUPERVISOR", "global-integrity");
  ...
  const reader = fixture(t, "SUPERVISOR", "KINGPEPE_FROST_A", d);
  const port = await reader.server.listen(input => authority.handle(input)), guard = new RemoteIntegrityGuard({ ipc: reader.client, port });
  const first = await guard.status(h("mainnet-preparation-status"));
  assert.equal(first.policyState, "PAUSED_POLICY"); assert.equal(first.state, "PAUSED_POLICY");
  await assert.rejects(guard.assertRunning(h("must-not-sign"), "FROST_SIGN"));
  await authority.close();
  const reopenStore = new WindowsProtectedStore(opts);
  assert.throws(() => new ProtectedIntegrityAuthority(reopenStore), /IntegrityExplicitEnrollmentRequired/);
  await assert.rejects(ProtectedIntegrityAuthority.openMainnetPaused(reopenStore, { ...d, keyEpoch: 2 }), /MainnetProtectedDeploymentRejected/);
  authority = await ProtectedIntegrityAuthority.openMainnetPaused(reopenStore, d);
  assert.equal(authority.status().state, "PAUSED_POLICY"); assert(BigInt(authority.status().generation) > BigInt(first.generation));
  const detector = fixture(t, "SUPERVISOR", "RECONCILIATION", d);
  const detectorPort = await detector.server.listen(input => authority.handle(input));
  const detectorGuard = new RemoteIntegrityGuard({ ipc: detector.client, port: detectorPort });
  const stopped = await detectorGuard.report(h("isolated-mismatch"), "CONFIRMED_RESERVE_DEFICIT", h("isolated-public-evidence"));
  assert.equal(stopped.state, "HARD_STOP_INTEGRITY"); assert.equal(stopped.incidentCount, 1);
  await authority.close(); authority = await ProtectedIntegrityAuthority.openMainnetPaused(new WindowsProtectedStore(opts), d);
  assert.equal(authority.status().state, "HARD_STOP_INTEGRITY"); assert.equal(authority.status().incidentCount, 1);
...
test("Mainnet protected fee payer and Solana outbox remain paused and cannot reuse TEST contexts", async t => {
  ...
  const authority = await ProtectedIntegrityAuthority.createMainnetPaused(bound("mainnet-claim-authority", "SUPERVISOR", "global-integrity"), d);
  cleanups.get(root).push(() => authority.close());
  const guard = async role => {
    const p = fixture(t, "SUPERVISOR", role, d), port = await p.server.listen(input => authority.handle(input));
    return new RemoteIntegrityGuard({ ipc: p.client, port });
  };
  const endpoint = "https://rpc.example.invalid/mainnet-claim-fixture", calls = [];
  t.mock.method(globalThis, "fetch", async (_url, init) => {
    assert.equal(_url, endpoint); const { id, method, params } = JSON.parse(init.body); calls.push(method); let result;
    if (method === "getGenesisHash") result = d.solanaGenesis;
    else if (method === "getBlockHeight") { assert.equal(params[0].commitment, "finalized"); result = 10; }
    else if (method === "getMultipleAccounts") { assert.equal(params[1].commitment, "finalized"); result = { context: { slot: 10 }, value: f.accounts().accounts.slice(0, -2) }; }
    else throw new Error("MainnetFixtureMustNotBroadcast");
    return Response.json({ jsonrpc: "2.0", id, result });
  });
  const payerGuard = await guard("FEE_PAYER"), payerOptions = bound("mainnet-claim-payer", "FEE_PAYER", "fee-payer-seed");
  const payerStore = f.withFeePayerSeedForTest(bytes => WindowsProtectedStore.create(payerOptions, bytes));
  const payerInput = { store: payerStore, integrity: payerGuard, policy: f.policy, endpoint };
  await assert.rejects(ProtectedDepositFeePayer.open(payerInput));
  const payer = await ProtectedDepositFeePayer.openMainnet(payerInput); cleanups.get(root).push(() => payer.close());
  const packet = await f.delivery(), { preparedTransactionBase64, ...intent } = packet;
  await assert.rejects(payer.prepare(intent), /IpcRequestRejected/);
  assert.equal(authority.status().state, "PAUSED_POLICY"); assert(!calls.includes("sendTransaction"));
  const relayGuard = await guard("RELAYER"), outboxOptions = bound("mainnet-claim-outbox", "RELAYER", "solana-deposit-outbox");
  const initial = initialSolanaDepositOutbox(f.policy), outboxStore = WindowsProtectedStore.create(outboxOptions, initial); initial.fill(0);
  const outboxInput = { store: outboxStore, integrity: relayGuard, policy: f.policy, endpoint, productionBroadcastAuthorized: false };
  await assert.rejects(ProtectedSolanaDepositOutbox.open(outboxInput));
  const outbox = await ProtectedSolanaDepositOutbox.openMainnet(outboxInput); cleanups.get(root).push(() => outbox.close());
  await assert.rejects(outbox.enqueue(packet), /IpcRequestRejected/);
  const stored = outboxStore.read(); try { assert.deepEqual(decodeSolanaDepositOutbox(stored.payload, f.policy).records, []); } finally { stored.payload.fill(0); }
  for (const mutate of [p => { p.operationPolicy.nativeGenesis = REGTEST_GENESIS; }, p => { p.operationPolicy.keyEpoch++; }, p => { p.operationPolicy.mint = h("substituted-mint"); }]) {
    const policy = structuredClone(f.policy); mutate(policy);
    await assert.rejects(ProtectedDepositFeePayer.openMainnet({ ...payerInput, policy }));
    await assert.rejects(ProtectedSolanaDepositOutbox.openMainnet({ ...outboxInput, policy }));
  }
});
...
test("Mainnet protected forward composition opens only with its exact deployment and remains paused", async t => {
  ...
  const authority = await ProtectedIntegrityAuthority.createMainnetPaused(bound("forward-authority", "SUPERVISOR", "global-integrity"), d);
  cleanups.get(root).push(() => authority.close());
  const guardFor = async role => {
    const p = fixture(t, "SUPERVISOR", role, d), port = await p.server.listen(input => authority.handle(input));
    return new RemoteIntegrityGuard({ ipc: p.client, port });
  };
  const guard = await guardFor("BRIDGE_VALIDATOR"), relayGuard = await guardFor("RELAYER");
  let calls = 0;
  ...
  const clients = [["COORDINATOR", ProtectedSweepJobClient, d], ["RELAYER", ProtectedNativeSweepClient, d],
    ...
    ["ATTESTER_A", ProtectedDepositAttesterClient, f.policy], ["ATTESTER_B", ProtectedDepositAttesterClient, f.policy]].map(([role, Type, policy]) => {
    const tr = fixture(t, role, "BRIDGE_VALIDATOR", d); tr.client.request = noNetwork;
    return new Type({ ipc: tr.client, port: 23456, role, policy });
  });
  const bookOptions = bound("forward-book", "BRIDGE_VALIDATOR", "deposit-operations"), bookBytes = initialDepositOperationState(d);
  const bookStore = WindowsProtectedStore.create(bookOptions, bookBytes); bookBytes.fill(0);
  const book = await ProtectedDepositOperationJournal.open({ store: bookStore, integrity: guard, policy: d });
  cleanups.get(root).push(() => book.close());
  await assert.rejects(book.retainFinalizedCredit(h("unproven"), { receipt: { proofVerified: true, reserveBasis: { genesis: d.nativeGenesis } } }),
  ...
  await assert.rejects(ProtectedNativeSweepOutbox.open(relayInput), /ExplicitNetworkRequired/);
  await assert.rejects(ProtectedNativeSweepOutbox.openMainnet({ ...relayInput, productionBroadcastAuthorized: undefined }), /AdmissionRequired/);
  await assert.rejects(ProtectedNativeSweepOutbox.openMainnet({ ...relayInput, nativeVerifier: wrongVerifier }), /NetworkMismatch/);
  const relay = await ProtectedNativeSweepOutbox.openMainnet(relayInput); cleanups.get(root).push(() => relay.close());
  assert.equal((await relay.runOne()).state, "WAITING_FOR_DEPENDENCY");
  await assert.rejects(relay.enqueue({ plan: { operationId: h("unproven") }, signedTransactionHex: "00" }));
  for (const mutate of [p => { p.nativeGenesis = REGTEST_GENESIS; }, p => { p.keyEpoch++; }, p => { p.mint = h("wrong-mint"); }]) {
    const op = structuredClone(d); mutate(op);
    await assert.rejects(ProtectedNativeSweepOutbox.openMainnet({ ...relayInput, policy: op }));
    await assert.rejects(ProtectedDepositController.openMainnet({ ...input,
      policy: { ...policy, deliveryPolicy: { ...f.policy, operationPolicy: op } } }));
  }
  assert.equal(authority.status().state, "PAUSED_POLICY"); assert.equal(calls, 0);
});
...
test("Mainnet A+B runtime and coordinator journal remain paused without economic nonce or signing records", async t => {
  const d = mainnetFixtureDeployment(), root = temporary(t);
  const bound = (name, role, purpose) => {
    const opts = options(root, name, role, purpose);
    Object.assign(opts.context, { environment: "mainnet", nativeGenesis: d.nativeGenesis, solanaDeployment: d.solanaDeployment, keyEpoch: d.keyEpoch });
    return opts;
  };
  const dkgPolicy = { environment: "mainnet", nativeNetwork: "mainnet", nativeGenesisHash: d.nativeGenesis, solanaGenesis: d.solanaGenesis,
    solanaDeployment: d.solanaDeployment, bridgeProgramId: d.managerProgramId, transceiverProgramId: d.transceiverProgramId, mint: d.mint, keyEpoch: d.keyEpoch,
    productionReady: false, productionSigningAuthorized: false, productionBroadcastAuthorized: false, mainnetActivation: "DISABLED" };
  const participantStates = REQUIRED_FROST_SIGNERS.map(role => bound(role, role, "frost-state"));
  const prepared = await prepareMainnetFrostParticipants({ policy: dkgPolicy, participantStates });
  assert(prepared.separateProcesses && prepared.threshold === 2);
  const reserve = "5120" + prepared.aggregateTweakedXOnlyPublicKey;
  const intent = { protocol: FROST_SIGNING_INTENT_PROTOCOL, mode: FROST_SIGNING_MODE, purpose: "RESERVE_SWEEP", nativeNetwork: "mainnet",
    nativeGenesisHash: d.nativeGenesis, solanaDeployment: d.solanaDeployment, bridgeProgramId: d.managerProgramId, transceiverProgramId: d.transceiverProgramId, mint: d.mint, keyEpoch: 1,
    signingRequestId: h("mainnet-fixture-request"), operationId: h("mainnet-fixture-operation"),  proofFingerprint: h("mainnet-fixture-proof"),
    unsignedNativeTransactionId: h("mainnet-fixture-transaction"), transactionCommitment: h("mainnet-fixture-commitment"), signingInputIndex: 0,
    taprootSighashHex: h("mainnet-fixture-sighash"), recipientScriptPubKeyHex: reserve, amountAtomic: "1000", feeAtomic: "10", changeScriptPubKeyHex: reserve,
    changeAtomic: "0", inputOutpoints: [h("mainnet-fixture-input") + ":0"], outputCommitments: [h("mainnet-fixture-output")], reserveCommitment: h("mainnet-fixture-reserve"),
    pauseDeposits: false, hardStop: false };
  const policy = createMainnetNativeSigningPolicy({ ...dkgPolicy, activationMode: "CONTROLLED", productionSigningAuthorized: true, productionBroadcastAuthorized: true,
    productionLimitPolicy: "UNBOUNDED_BY_TEAM_DECISION", controlledOperationIds: [intent.operationId], reserveScriptPubKeyHex: reserve, authorizedOperations: [intent],
    // Isolated fixture ceilings; never copied into the actual production profile.
    nativeFeePolicy: { policy: "DYNAMIC_NODE_ESTIMATE_WITH_CAP", minimumRelayAtomicPerKvB: "100", maximumAtomicPerKvB: "1000", maximumFeeAtomic: "100" } });
  const authorityOptions = bound("mainnet-frost-supervisor", "SUPERVISOR", "global-integrity");
  const authority = await ProtectedIntegrityAuthority.createMainnetPaused(authorityOptions, d); cleanups.get(root).push(() => authority.close());
  const peerGuard = async role => {
    const p = fixture(t, "SUPERVISOR", role, d), port = await p.server.listen(input => authority.handle(input));
    return new RemoteIntegrityGuard({ ipc: p.client, port });
  };
  const signers = [], states = [], verificationCounts = [0, 0];
  for (const [index, stateOptions] of participantStates.entries()) {
    const role = REQUIRED_FROST_SIGNERS[index], base = new WindowsProtectedFrostStateStore(new WindowsProtectedStore(stateOptions), role), integrity = await peerGuard(role);
    const runtime = await openProtectedNativeSigner({ base, policy, integrity, nativeEvidenceValidator: async value => {
      verificationCounts[index]++; return { digestHex: value.proofFingerprint }; // Fixture only; the pause must still prevent any nonce.
    } });
    states.push(base); cleanups.get(root).push(() => runtime.close());
    const p = fixture(t, role, "COORDINATOR", d), port = await p.server.listen(runtime.handler);
    const signer = new ProtectedRemoteFrostPeer({ ipc: p.client, port, signerId: role }); signers.push(signer);
    assert.throws(() => signer.assertDeployment({ environment: "localnet", nativeGenesis: REGTEST_GENESIS, solanaDeployment: d.solanaDeployment, keyEpoch: 1 }), /IpcSignerDeploymentMismatch/);
    await assert.rejects(signer.signingCommitment(createNativeFrostSigningRequest(intent)), /IpcRequestRejected/);
    const state = base.load(); assert.equal(Object.keys(state.signing).length, 0); assert.equal(Object.keys(state.nonceTombstones).length, 0);
  }
  assert.deepEqual(verificationCounts, [1, 1]);
  const integrity = await peerGuard("COORDINATOR"), key = { publicPackage: prepared.publicPackage, aggregateTweakedXOnlyPublicKey: prepared.aggregateTweakedXOnlyPublicKey };
  const journalOptions = bound("mainnet-coordinator-journal", "COORDINATOR", "coordinator-signing"), bytes = initialCoordinatorSigningState(key);
  const store = WindowsProtectedStore.create(journalOptions, bytes); bytes.fill(0);
  await assert.rejects(ProtectedCoordinatorSigningJournal.open({ store, integrity, ...key }), /ProtectedCoordinatorExplicitEnvironmentRequired/);
  let journal = await ProtectedCoordinatorSigningJournal.openMainnet({ store, integrity, ...key, deployment: d });
  cleanups.get(root).push(() => journal.close());
  for (const mutation of [{ nativeNetwork: "regtest" }, { bridgeProgramId: h("wrong-program") }, { transceiverProgramId: h("wrong-transceiver") }, { mint: h("wrong-mint") }])
    assert.throws(() => journal.lookup({ ...intent, ...mutation }), /CoordinatorJournalContextMismatch/);
  for (let attempt = 0; attempt < 2; attempt++) {
    const coordinator = new NativeFrostCoordinator({ signers, ...key, integrity, signingJournal: journal });
    await assert.rejects(coordinator.signAutomaticallyOverIpc(intent), /IpcRequestRejected/);
    assert.equal(journal.lookup(intent), null); assert.deepEqual(verificationCounts, [1, 1]);
    await journal.close(); journal = await ProtectedCoordinatorSigningJournal.openMainnet({ store: new WindowsProtectedStore(journalOptions), integrity, ...key, deployment: d });
  }
...
async function protectedCoordinatorFixture(t, sweepJobFixture = false) {
  ...
  const intent = { protocol: FROST_SIGNING_INTENT_PROTOCOL, mode: FROST_SIGNING_MODE, purpose: "RESERVE_SWEEP", nativeNetwork: "regtest",
    ...
    signingRequestId: h("request"), operationId: h("operation"),  proofFingerprint: h("proof"), unsignedNativeTransactionId: h("transaction"),
...
test("protected reserve sweep aggregate persists across coordinator restart without another signature", async t => {
  // Actual DPAPI, TLS and FROST; this fixture models chain evidence. The
  // separate real-chain reserve sweep remains the Native-acceptance proof.
  ...
  assert.throws(() => journal.lookup({ ...f.intent, amountAtomic: "1001" }), /CoordinatorJournalRequestChanged/u);
  assert.throws(() => journal.lookup({ ...f.intent, purpose: "RESERVE_MIGRATION" }), /unsupported Native FROST signing purpose/u);
});
...
for (const role of ["ATTESTER_A", "ATTESTER_B"]) test("Mainnet protected attester identity and journal stay gated while paused: " + role, async t => {
  const d = mainnetFixtureDeployment(), peer = fixture(t, "SUPERVISOR", role, d), seed = randomBytes(32);
  const bound = (name, purpose, instanceId = h(name), namedRole = role) => {
    const opts = options(peer.root, name, namedRole, purpose, instanceId);
    Object.assign(opts.context, { environment: "mainnet", nativeGenesis: d.nativeGenesis, solanaDeployment: d.solanaDeployment, keyEpoch: d.keyEpoch });
    return opts;
  };
  const authorityOptions = bound("mainnet-attester-supervisor", "global-integrity", h("mainnet-attester-supervisor"), "SUPERVISOR");
  const authority = await ProtectedIntegrityAuthority.createMainnetPaused(authorityOptions, d);
  cleanups.get(peer.root).push(() => authority.close());
  let refusal;
  const port = await peer.server.listen(input => {
    try { return authority.handle(input); } catch (error) { refusal = error.message; throw error; }
  }), guard = new RemoteIntegrityGuard({ ipc: peer.client, port });
  const policy = { role, attesterPublicKeyHex: Buffer.from(ed25519.getPublicKey(seed)).toString("hex"), protocolId: d.protocolId,
    ...
    keyEpoch: d.keyEpoch, policyEpoch: 1, acceptedNativeTrust: ["LOCALLY_VALIDATED_CHAIN_STATE"], depositsPaused: false, hardStop: false };
  const seedOptions = bound("mainnet-attester-seed", "attester-seed"), store = WindowsProtectedStore.create(seedOptions, seed); seed.fill(0);
  cleanups.get(peer.root).push(() => store.close());
  assert.throws(() => ProjectAttester.fromWindowsProtectedStore({ store, role, policy, environment: "mainnet", solanaGenesis: d.solanaGenesis }), /ProtectedAttesterLocalOnly/);
  for (const mutation of [{ mintHex: h("other-mint") }, { nativeGenesisHex: REGTEST_GENESIS }, { nativeNetwork: 8000111 },
    { managerProgramIdHex: h("other-program") }, { transceiverProgramIdHex: h("other-transceiver") }, { solanaDeploymentHex: h("other-deployment") },
    { keyEpoch: 2 }, { policyEpoch: 0 }, { acceptedNativeTrust: ["RPC_ONLY"] }])
    assert.throws(() => ProjectAttester.fromMainnetProtectedStore({ store, role, policy: { ...policy, ...mutation }, deployment: d }), /MainnetAttesterPolicyBindingRejected/);
  assert.throws(() => ProjectAttester.fromMainnetProtectedStore({ store, role, policy: { ...policy, attesterPublicKeyHex: h("other-attester") }, deployment: d }), /AttesterPolicyIdentityMismatch/);
  const attester = ProjectAttester.fromMainnetProtectedStore({ store, role, policy, deployment: d });
  cleanups.get(peer.root).push(() => attester.close()); assert.equal(attester.publicKeyHex, policy.attesterPublicKeyHex);
  const journalOptions = bound("mainnet-attester-authorizations", "attester-authorizations", seedOptions.context.instanceId);
  const bytes = ProtectedAttesterAuthorizationJournal.initialState(attester), journalStore = WindowsProtectedStore.create(journalOptions, bytes); bytes.fill(0);
  let journal = await ProtectedAttesterAuthorizationJournal.open({ attester, integrity: guard, store: journalStore });
  cleanups.get(peer.root).push(() => journal.close());
  ...
  const encoded = Buffer.from(encodeCanonicalBridgeMessage({ ...decodeCanonicalBridgeMessage(vector.encodedHex), operationId: undefined,
    deployment: d, policyEpoch: 1, keyEpoch: 1, validFrom: now - 5n, validUntil: now + 300n })).toString("hex"), decoded = decodeCanonicalBridgeMessage(encoded);
  const evidence = { trust: "LOCALLY_VALIDATED_CHAIN_STATE", nativeNetwork: d.nativeNetwork, nativeGenesisHash: d.nativeGenesis,
    ...
    solanaRecipientHex: decoded.destinationHex, evidenceDigestHex: decoded.evidenceDigestHex, reserveAllocationIdHex: h("mainnet-attester-isolated-allocation"),
  ...
  let verifiedCalls = 0, signingCalls = 0;
  const sign = ProjectAttester.prototype.signDepositCredit;
  t.mock.method(ProjectAttester.prototype, "signDepositCredit", function(...args) { signingCalls++; return sign.apply(this, args); });
  const verifyNativeDeposit = async () => { verifiedCalls++; return { operationIdHex: decoded.operationIdHex, messageDigestHex: decoded.messageDigestHex, evidence }; };
  const input = { peerRole: "BRIDGE_VALIDATOR", method: "attestDeposit", operationId: decoded.operationIdHex,
    payload: { encodedMessageHex: encoded, rawEvidence: { proofVerified: true } } };
  for (let attempt = 0; attempt < 2; attempt++) {
    const handler = attesterIpcHandler({ attester, integrity: guard, journal, verifyNativeDeposit });
    await assert.rejects(handler(input), /IpcRequestRejected/);
    assert.equal(refusal, "IntegrityAuthorizationStopped");
    const saved = new WindowsProtectedStore(journalOptions), read = saved.read();
    try { assert.equal(JSON.parse(read.payload.toString("utf8")).records.length, 0); } finally { read.payload.fill(0); saved.close(); }
    await journal.close(); journal = await ProtectedAttesterAuthorizationJournal.open({ attester, integrity: guard, store: new WindowsProtectedStore(journalOptions) });
  }
  assert.equal(verifiedCalls, 2); assert.equal(signingCalls, 0); assert.equal(authority.status().state, "PAUSED_POLICY");
  assert.notEqual(attester.evaluateDepositCredit({ encodedMessageHex: encoded, evidence: { ...evidence, finalitySatisfied: false } }).state, "VERIFIED_READY");
});
...
test("Mainnet protected deployment monitoring persists observations while paused and latches a mismatch", async t => {
  const { manifest, snapshot } = mainnetDeploymentFixture(), hex = v => Buffer.from(base58Decode(v)).toString("hex");
  const d = { environment: "mainnet", protocolId: manifest.config.protocolId, nativeNetwork: manifest.config.nativeNetwork,
    nativeGenesis: manifest.nativeGenesisHex, solanaGenesis: manifest.solanaGenesis, solanaDeployment: manifest.solanaDeploymentHex,
    managerProgramId: hex(manifest.manager.id), transceiverProgramId: hex(manifest.transceiver.id), mint: hex(manifest.mint.id), keyEpoch: manifest.config.keyEpoch };
  const peer = fixture(t, "SUPERVISOR", "SOLANA_OBSERVER", d), root = peer.root;
  const boundOptions = (name, role, purpose) => {
    const opts = options(root, name, role, purpose);
    Object.assign(opts.context, { environment: "mainnet", nativeGenesis: d.nativeGenesis, solanaDeployment: d.solanaDeployment, keyEpoch: d.keyEpoch });
    return opts;
  };
  const authorityOptions = boundOptions("monitor-authority", "SUPERVISOR", "global-integrity");
  let authority = await ProtectedIntegrityAuthority.createMainnetPaused(authorityOptions, d);
  cleanups.get(root).push(() => authority.close());
  const port = await peer.server.listen(input => authority.handle(input)), guard = new RemoteIntegrityGuard({ ipc: peer.client, port });
  let response = snapshot, unavailable = false;
  t.mock.method(globalThis, "fetch", async (_url, init) => {
    if (unavailable) throw new Error("IsolatedRpcUnavailable");
    const q = JSON.parse(init.body);
    assert(["getGenesisHash", "getMultipleAccounts"].includes(q.method));
    return Response.json({ jsonrpc: "2.0", id: q.id, result: q.method === "getGenesisHash" ? response.genesis :
      { context: { slot: response.slot }, value: response.accounts } });
  });
  const rpc = LocalDeploymentRpc.createMainnet({ endpoint: "https://rpc.example.invalid", expectedGenesis: SOLANA_MAINNET_GENESIS });
  const opts = boundOptions("mainnet-progress", "SOLANA_OBSERVER", "chain-progress"), initial = ProtectedSolanaDeploymentMonitor.initialProgress(manifest);
  const store = WindowsProtectedStore.create(opts, initial); initial.fill(0);
  let monitor = await ProtectedSolanaDeploymentMonitor.open({ manifest, rpc, integrity: guard, store });
  cleanups.get(root).push(() => monitor.close());
  assert.equal((await monitor.poll()).state, "OBSERVED_MATCH"); assert.equal(authority.status().state, "PAUSED_POLICY");
  await monitor.close(); monitor = await ProtectedSolanaDeploymentMonitor.open({ manifest, rpc, integrity: guard, store: new WindowsProtectedStore(opts) });
  unavailable = true; assert.equal((await monitor.poll()).state, "WAITING_FOR_DEPENDENCY");
  assert.equal(authority.status().incidentCount, 0); unavailable = false;
  response = structuredClone(snapshot); response.slot++;
  const data = Buffer.from(response.accounts[5].data[0], "base64"); data[13] ^= 1; response.accounts[5].data[0] = data.toString("base64");
  assert.equal((await monitor.poll()).state, "HARD_STOP_INTEGRITY"); assert.equal(authority.status().incidentCount, 1);
  await monitor.close(); await authority.close(); authority = await ProtectedIntegrityAuthority.openMainnetPaused(new WindowsProtectedStore(authorityOptions), d);
  monitor = await ProtectedSolanaDeploymentMonitor.open({ manifest, rpc, integrity: guard, store: new WindowsProtectedStore(opts) });
  response = { ...snapshot, slot: 12 }; assert.equal((await monitor.poll()).state, "HARD_STOP_INTEGRITY");
  assert.equal(authority.status().incidentCount, 1);
});
...
test("Mainnet protected Native observer keeps unavailable evidence paused and retains a reorg incident", async t => {
  const f = mainnetNativeProgressFixture(), d = f.expectedPolicy.deployment, peer = fixture(t, "SUPERVISOR", "NATIVE_OBSERVER", d);
  const bound = (name, role, purpose) => {
    const opts = options(peer.root, name, role, purpose);
    Object.assign(opts.context, { environment: "mainnet", nativeGenesis: d.nativeGenesis, solanaDeployment: d.solanaDeployment, keyEpoch: d.keyEpoch });
    return opts;
  };
  const authorityOptions = bound("native-mainnet-authority", "SUPERVISOR", "global-integrity");
  let authority = await ProtectedIntegrityAuthority.createMainnetPaused(authorityOptions, d);
  cleanups.get(peer.root).push(() => authority.close());
  const port = await peer.server.listen(input => authority.handle(input)), guard = new RemoteIntegrityGuard({ ipc: peer.client, port });
  const opts = bound("native-mainnet-progress", "NATIVE_OBSERVER", "chain-progress");
  const initial = Buffer.from(JSON.stringify(f.stored)), store = WindowsProtectedStore.create(opts, initial); initial.fill(0);
  const verifier = LocalNativeEvidenceVerifier.createMainnet({ rpc: { async getSourceSnapshot() { throw new Error("IsolatedNodeUnavailable"); } }, executable: "unused-fixture" });
  let monitor = await ProtectedNativeIntegrityMonitor.openMainnet({ expectedPolicy: f.expectedPolicy, verifier, integrity: guard, store });
  cleanups.get(peer.root).push(() => monitor.close());
  assert.equal((await monitor.poll()).state, "WAITING_FOR_DEPENDENCY");
  assert.equal(authority.status().state, "PAUSED_POLICY"); assert.equal(authority.status().incidentCount, 0);
  await monitor.close();
  // Isolated authenticated incident fixture; never alter retained Mainnet state.
  f.chain.headerHashes[4988] = h("isolated-accepted-mainnet-deposit-reorg"); f.chain.chainworkHex = "00".repeat(31) + "20";
  const stopped = compareNativeProgress(f.stored, f.chain, f.expectedPolicy, f.now), nextStore = new WindowsProtectedStore(opts);
  const read = nextStore.read(), payload = Buffer.from(JSON.stringify(stopped.progress));
  try { nextStore.write(payload, read.revision); } finally { read.payload.fill(0); payload.fill(0); }
  monitor = await ProtectedNativeIntegrityMonitor.openMainnet({ expectedPolicy: f.expectedPolicy, verifier, integrity: guard, store: nextStore });
  assert.equal((await monitor.poll()).state, "HARD_STOP_INTEGRITY"); assert.equal(authority.status().incidentCount, 1);
  await monitor.close(); await authority.close();
  authority = await ProtectedIntegrityAuthority.openMainnetPaused(new WindowsProtectedStore(authorityOptions), d);
  monitor = await ProtectedNativeIntegrityMonitor.openMainnet({ expectedPolicy: f.expectedPolicy, verifier, integrity: guard, store: new WindowsProtectedStore(opts) });
  assert.equal((await monitor.poll()).state, "HARD_STOP_INTEGRITY"); assert.equal(authority.status().incidentCount, 1);
});
