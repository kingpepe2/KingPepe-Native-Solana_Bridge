...
export async function solanaDeliveryFixture({ mainnet = false } = {}) {
  ...
  if (mainnet) {
    const deployment = mainnetDeploymentFixture(4);
    f.manifest = deployment.manifest; f.snapshot = deployment.snapshot;
    Object.assign(f.policy, { environment: "mainnet", nativeGenesis: f.manifest.nativeGenesisHex, solanaGenesis: f.manifest.solanaGenesis,
      solanaDeployment: f.manifest.solanaDeploymentHex, protocolId: f.manifest.config.protocolId, nativeNetwork: f.manifest.config.nativeNetwork,
      minimumConfirmations: 12, csvDelayBlocks: 1440, maximumAmountAtomic: "18446744073709551615" });
  }
  ...
  const p = f.policy, encoded = encodeCanonicalBridgeMessage({ action: "DepositClaim", direction: "NativeToSolana",
    ...
    depositOutpoint: { txid: h(digest("disposable Solana-delivery deposit")), vout: 0 },
  ...
  async function delivery(kind = "RECEIPT", generation = 0) {
    ...
    const tx = await build({ environment: p.environment, cluster: p.environment, solanaGenesis: p.solanaGenesis,
      ...(mainnet ? { protocolId: p.protocolId, nativeNetwork: p.nativeNetwork, nativeGenesis: p.nativeGenesis, solanaDeployment: p.solanaDeployment } : {}),
      managerProgramIdHex: p.managerProgramId, transceiverProgramIdHex: p.transceiverProgramId,
  ...
  function accounts({ receipt = false, claim = false } = {}) {
    ...
    const c = Buffer.alloc(211); c.write("KPBCLM01"); c[8] = 2; h(message.operationIdHex).copy(c, 9); h(message.messageDigestHex).copy(c, 41);
