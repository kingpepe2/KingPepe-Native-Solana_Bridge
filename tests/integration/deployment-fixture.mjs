...
import { DEPLOYMENT_MONITOR_PROTOCOL, UPGRADEABLE_LOADER, deploymentDigest } from "../../services/solana-observer/deployment-integrity.mjs";
...
export function deploymentFixture() {
  ...
  const program = id => ({ id, loader: UPGRADEABLE_LOADER, programData: findProgramAddress([decode(id)], decode(UPGRADEABLE_LOADER)).base58,
    upgradeAuthority: k("test-upgrade"), deploymentSlot: "0", binaryLength: binary.length, binaryHash: deploymentDigest(binary), dataLength: binary.length + 45 });
  ...
  const data = [manifest.manager, manifest.transceiver].map(p => { const b = Buffer.alloc(p.dataLength); b.writeUInt32LE(3); b[12] = 1; decode(p.upgradeAuthority).copy(b, 13); binary.copy(b, 45); return record(UPGRADEABLE_LOADER, false, b); });
