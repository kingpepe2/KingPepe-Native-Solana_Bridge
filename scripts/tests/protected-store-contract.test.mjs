...
const context = () => ({ role: "BURN_SIGNER", purpose: "native-burn-key", serviceSid: "S-1-5-21-1-2-3-1001",
...
test("protected context has exact immutable fields and rejects getters/proxies", () => {
  ...
  assert.equal(copy.role, "BURN_SIGNER"); assert(Object.isFrozen(copy));
...
test("protected context binds role purpose identity network deployment instance and epoch", () => {
  ...
  for (const change of [{ role: "ATTESTER_A", purpose: "attester-seed" }, { purpose: "service-auth" }, { serviceSid: "S-1-5-21-1-2-3-1002" },
...
test("protected adapters reject caller-forged store capabilities", () => {
  assert.throws(() => assertWindowsProtectedStore({ context: context(), read() { throw new Error("MUST_NOT_READ"); } }, "BURN_SIGNER", "native-burn-key"), /ProtectedStoreRoleMismatch/u);
});
