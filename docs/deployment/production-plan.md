...
Do not reuse the reserve journal, reserve signing authority, TEST keys or historical attestation. Native Mainnet requires source-derived actual genesis/consensus admission, unique per-operation addresses, exact two-class-input burn construction, independently verified twelve-confirmation deposit and burn finality, single-use address observation, operational fee coins, burn evidence and restart recovery. The separate [Mainnet service and activation lifecycle](mainnet-runtime.md) use protected production credentials and a dedicated burn journal. TEST resume cannot enable Mainnet. Actual dedicated service identity and portable offline recovery still require deployment-specific verification.
