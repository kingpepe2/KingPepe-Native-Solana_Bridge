...
Do not reuse the reserve journal, reserve signing authority, TEST keys or historical attestation. Native Mainnet requires source-derived actual genesis/consensus admission, unique per-operation addresses, exact two-class-input burn construction, independently verified twelve-confirmation deposit and burn finality, single-use address observation, operational fee coins, burn evidence and restart recovery. Mainnet-aware adapters are reviewed and tested. The existing TEST service composition and TEST resume method remain TEST-only; production service/custody/journal composition still needs its own verification.
