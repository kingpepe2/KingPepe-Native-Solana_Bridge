...
- Burn custody is SINGLE_KEY_ACCEPTED_RISK, server-side DPAPI and strict ACL, isolated from Explorer. Windows Service/SCM hosting is not required; verify the actual private automatic signer process and restart behavior. Do not require a reboot for the obsolete SCM proof service. No FROST burn/sweep key. Keep Noble curves 2.3.0 for BIP340/BIP341 and Ed25519. Never reuse burn, payer, upgrade or attester keys across roles.
- OFFLINE_RECOVERY_BACKUP=NOT_REQUIRED_BY_TEAM_DECISION; RECOVERY_RISK=ACCEPTED_BY_KINGPEPE_TEAM. Loss of the only usable burn signing material can leave confirmed but unburned deposits unavailable for automatic processing/recovery. Do not request offline provisioning as an activation gate, fabricate a backup or claim server-loss recovery. Journal/restart idempotency and all economic admission checks remain required.
