...
export async function runLocalDepositSecurityE2e(repoRoot) {
  ...
  return { ...result, localSecurity: { passed, pass: passed.length, fail: result.state === "COMPLETED" && passed.length === 3 ? 0 : 1,
    scope: "Real local-validator happy path, idempotent retry and duplicate-backing preflight rejection; not complete adversarial coverage." } };
}
