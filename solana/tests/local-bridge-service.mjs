...
async function child(file, mode) {
  ...
  try {
    ...
    const service = new LocalBridgeService({ ledger, depositWorker });
    ...
    if (mode === "user-api-intake") {
      ...
      try {
        ...
        const began = Date.now(), rssBefore = process.memoryUsage().rss;
        // Bounded isolated traffic against this disposable local service only.
        // Unknown transactions must not occupy persistent watch/fee slots.
        for (let i = 0; i < 128; i++) {
          await assert.rejects(client.submitNativeDeposit({
            request: c.publicDeposit.request, depositTxidHex: i.toString(16).padStart(64, "0"), depositVout: null }));
          if (i % 32 === 31) process.stderr.write(`INTAKE_IRRELEVANT_${i + 1}\n`);
        }
        for (let i = 0; i < 32; i++) await assert.rejects(client.submitNativeDeposit({
          request: c.publicDeposit.request, depositTxidHex: c.publicDeposit.depositTxidHex, depositVout: null, unexpected: true }));
        assert.deepEqual(ledger.checkpoint(), before); assert.equal(ledger.serviceDepositRequests().length, 0);
        ...
        process.stderr.write("INTAKE_INVALID_COMPLETE\n");
        ...
        for (let i = 0; i < 128; i++) {
          await client.submitNativeDeposit(input);
          if (i % 32 === 31) process.stderr.write(`INTAKE_DUPLICATE_${i + 1}\n`);
        }
        ...
        const boundedIntake = { irrelevant: 128, malformed: 32, duplicates: 128, legitimate: 1,
          durationMs: Date.now() - began, rssBefore, rssAfter: process.memoryUsage().rss, pendingWatches: ledger.serviceDepositRequests().length };
        assert(boundedIntake.rssAfter - rssBefore < 256 * 1024 * 1024, "BOUNDED_INTAKE_MEMORY_LIMIT");
        process.stdout.write(JSON.stringify({ state: "USER_API_INTAKE_PASS", boundedIntake }) + "\n"); return;
...
export async function runLocalBridgeService(onCheck = () => {}, { restoreSnapshot } = {}) {
  ...
  const result = await withLocalE2eInfrastructure({ repoRoot }, async context => {
    ...
    try {
      ...
      created.configureEconomicLimits({ scope: "TEST_ONLY", windowSeconds: 60,
        maxMintPerTransfer: config.amountAtomic,
        maxMintPerWindow: (2n * BigInt(config.amountAtomic)).toString() });
      created.close();
      const once = async mode => {
        diskGuard();
        try {
          const r = await promisify(execFile)(process.execPath, [self, "--service-child", file, mode ?? "tick"],
            { cwd: repoRoot, timeout: mode === "user-api-intake" ? 240000 : 90000, maxBuffer: 65536, windowsHide: true });
          return JSON.parse(r.stdout);
        } catch (error) {
          // Child stderr can contain private fixture paths. Retain only a source
          // location and the bounded-workload timeout classification.
          if (error.killed) {
            const phase = error.stderr?.match(/INTAKE_[A-Z0-9_]+/gu)?.at(-1) ?? "NO_PROGRESS";
            throw new Error("LOCAL_SERVICE_CHILD_TIMEOUT:" + phase);
          }
          const location = error.stderr?.match(/local-bridge-service\.mjs:[0-9]+:[0-9]+/u)?.[0];
          if (location) error.stack = "LocalServiceChildError at " + location;
          throw error;
        }
      };
      ...
      const intake = await once("user-api-intake"); assert.equal(intake.state, "USER_API_INTAKE_PASS");
      ...
      useLedger(l => { assert.equal(l.bridgeSnapshot().bridgeIssuedAtomic, "0"); l.pause(); });
      ...
      assert.equal(mintPaused.status.accounting.bridgeIssuedAtomic, config.amountAtomic);
      ...
      assert.equal(bank.mintSupplyAtomic, config.amountAtomic);
      report = { sourceSha, worktreeDirty: execFileSync("git", ["status", "--porcelain"], { cwd: repoRoot, encoding: "utf8" }).trim().length > 0,
        nativeToSolana: "COMPLETED", noPerTransferKingPepeTeamApproval: true, accounting: resumed.status.accounting,
        boundedIntake: { ...intake.boundedIntake, legitimateOperation: "COMPLETED", reconciliation: resumed.result.reconciliation.state },
        economicLimits: useLedger(l => l.economicLimitsStatus()) };
      return { state: "COMPLETED" };
    } catch (error) {
      const firstLine = error.message?.split("\n")[0];
      failure = /^[A-Z_a-z0-9: ]{1,160}$/u.test(firstLine) ? firstLine : "LOCAL_SERVICE_ASSERTION_FAILED";
  ...
  const complete = result.state === "LOCAL_E2E_BOOTSTRAP_READY" && report?.nativeToSolana === "COMPLETED" && !failure && passed.length === 16 && (!restoreSnapshot || recovery.length === 2);
