...
export async function testRecoveryPsbtWithNativeWallet(context) {
  ...
  const recoveryTransaction = parseNativeTransactionHex(finalized.hex);
  assert.equal(recoveryTransaction.txidHex, parseNativeTransactionHex(prepared.unsignedTransactionHex).txidHex);
  assert.equal(recoveryTransaction.outputs[0].amountAtomic, "999000");
  assert.equal(recoveryTransaction.outputs[0].scriptPubKeyHex, destinationInfo.scriptPubKey);
  assert.equal((await cli("testmempoolaccept", [JSON.stringify([finalized.hex])], true))[0].allowed, true);
  assert.equal(await cli("sendrawtransaction", [finalized.hex]), recoveryTransaction.txidHex);
  await userCli("generatetoaddress", ["1", miningAddress]);
  const received = await userCli("gettransaction", [recoveryTransaction.txidHex], true);
  assert.equal(received.txid, recoveryTransaction.txidHex); assert.equal(received.confirmations, 1);
  ...
  return Object.freeze({ pass: 4, fail: 0, checks: ["NATIVE_PSBT_DECODE_MATCHES_UNSIGNED_RECOVERY",
    "NATIVE_USER_WALLET_SIGNS_WITHOUT_KEY_EXPORT", "PSBT_RECOVERY_RECOGNIZED_BY_USER_WALLET",
