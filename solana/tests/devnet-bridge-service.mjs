...
import { LocalBridgeService } from "../../services/relayer/deposit-service.mjs";
...
try {
  ...
  assert(checkpoint === undefined || cycle !== undefined && checkpoint === "NATIVE_SWEEP_BROADCAST", "TestRecoveryCheckpointRejected");
  ...
  evidence.programSourceSha = record.sourceSha; evidence.borshSchemaVersion = 3;
  ...
  const policy = { ...partial, environment: "devnet", solanaGenesis: DEVNET_SOLANA_GENESIS, minimumSolanaSlot: String(manifest.minimumSlot),
    ...
    frostPublicKeyHex: publicPackage.aggregateTweakedXOnlyPublicKey, csvDelayBlocks: 1440, minimumConfirmations: 12,
  ...
  // Explicit small TEST values live in private operator configuration. Existing
  // journal policy cannot be replaced by a restart; Mainnet values are unset.
  ledger.configureEconomicLimits(publicUi?.economicLimits ?? JSON.parse(readFileSync(
    validateRuntimeFile(required("KINGPEPE_DEVNET_TEST_LIMITS"), repoRoot), "utf8")));
  ...
  const deploymentSeed = deployment?.payload.subarray(0, 32);
  const fundingKey = publicUi ? null : base58Encode(ed25519.getPublicKey(deploymentSeed));
  if (!publicUi) assert.equal(fundingKey, manifest.manager.upgradeAuthority);
  ...
  try {
    ...
    assert(available >= 60000000n, "ExistingDevnetTestFundingInsufficient");
    event("EXISTING_TEST_BALANCE_SUFFICIENT", { balanceLamports: available.toString(), airdropsRequested: 0 });
    for (const [label, address, amount] of [["runtime-fees", feePayer.publicKeyBase58, 50000000n]]) {
  ...
  async function observeHealth() {
    const reads = [() => nativeVerifier.observeChain(), async () => verifyDeploymentSnapshot(manifest, await solana.snapshot(manifest)), () => deposits.reconcile()];
  ...
  const service = new LocalBridgeService({ ledger, depositWorker: deposits });
  let feeFundingInputs;
  const api = new BridgeUserApi({ service, ledger, balances: new BridgeUserBalances({ nativeRpc, solanaRpc: solana, manifest }), depositFeeInputs: async () => {
  ...
  if (restoreReview) {
    ...
    const pending = ledger.serviceDeposit(boundary.operationId)?.operation;
    ...
    if (!(actual.confirmations >= policy.minimumConfirmations)) await nativeCli("generatetoaddress", [String(policy.minimumConfirmations), miner], wallet);
    await service.resumeAfterReview();
    assert.equal(signingRequests, 0); assert.equal(nativeBroadcasts.length, 0);
    assert.equal((await deposits.reconcile()).state, "MATCH");
  ...
  if (publicUi) {
    ...
    while (!interrupted) {
      diskGuard();
      const pending = ledger.serviceDeposits({ pendingOnly: true }).length;
      if (pending && Date.now() - lastMined >= 10000 && JSON.parse(await nativeCli("getrawmempool")).length > 0) {
        const height = Number(await nativeCli("getblockcount"));
        assert(height + policy.minimumConfirmations <= 4096, "PublicTestRegtestHeaderBudgetExhausted");
        await nativeCli("generatetoaddress", [String(policy.minimumConfirmations), miner], wallet); lastMined = Date.now();
      }
      const result = await service.tick();
      const state = result.reconciliation?.state ?? result.state;
      const observation = { state, bridgeState: service.status().state, reason: result.reason ?? service.status().pauseReason ?? null };
      if (json(observation) !== lastState) { event("PUBLIC_TEST_SERVICE_OBSERVATION", observation); lastState = json(observation); }
      await delay(5000);
    }
  ...
  const completedBefore = ledger.serviceDeposits().filter(v => v.state === "COMPLETED").map(v => v.operationId);
  if (soakSeconds) {
  ...
  await nativeCli("generatetoaddress", [String(policy.minimumConfirmations), miner], wallet);
  await api.submitNativeDeposit({ request, depositTxidHex, depositVout: null });
  let last;
  async function tickUntil(id) {
    const until = Date.now() + 15 * 60 * 1000;
    for (;;) {
      diskGuard(); assert(Date.now() < until, "DevnetServiceCompletionTimeout");
      const result = await service.tick(), state = service.status();
      const progress = json({ state: state.state, deposits: state.deposits, reconciliation: result.reconciliation?.state });
      ...
      const pendingBoundary = "NATIVE_SWEEP_BROADCAST";
      const pending = ledger.serviceDeposit(id)?.operation;
      ...
      if (JSON.parse(await nativeCli("getrawmempool")).length) await nativeCli("generatetoaddress", [String(policy.minimumConfirmations), miner], wallet);
      await delay(2000);
    }
  }
  evidence.nativeToSolana = await tickUntil(quote.operationId);
  ...
  if (networkEdges) {
    ...
    if (existsSync(replayEvidence)) evidence.replay = JSON.parse(readFileSync(replayEvidence));
      ...
      }, [manifest.mint.id, record.recipientSplAccount, manifest.config.bridgePda], credit.encodedMessageHex);
      evidence.replay = { status: "PASS", duplicateClaim };
  ...
  if (networkEdges) {
    ...
    if (existsSync(savedEdge)) evidence.nativeConflict = JSON.parse(readFileSync(savedEdge));
      ...
      evidence.nativeConflict = { status: "PASS", originalDeposit: depositTxidHex, conflictTxid: txid,
        walletFinalized: true, csvMature: true, nodeRejectedSpentInput: true, newMint: 0 };
  ...
  evidence.reconciliation = await deposits.reconcile(); assert.equal(evidence.reconciliation.state, "MATCH");
  const finalState = service.status();
  assert.equal(finalState.accounting.pendingMintAtomic, "0");
  ...
  if (soakSeconds) {
    evidence.traffic = { nativeToSolanaCount: Number(!completedBefore.includes(quote.operationId)),
      depositOperationId: quote.operationId };
    ...
    evidence.liveBorsh = { status: "PASS", schemaVersion: 3, transactions: live };
  ...
  if (soakSeconds) {
    ...
    evidence.edgeChecks = { pauseReviewedResume: "PASS", injectedRpcOutageAndRealReconnect: "PASS",
      noNewSigning: signingRequests === signingCount,
