...
export async function verifyDevnetDeployment(manifest, binaryDirectory) {
  ...
  check(manifest.scope === "DEVNET_TEST_ONLY" && manifest.solanaGenesis === DEVNET_SOLANA_GENESIS &&
    ...
    manifest.borshSchemaVersion === 3 && /^[0-9a-f]{40}$/u.test(manifest.sourceSha) &&
  ...
  const bridgeExpected = encodeBridgeAbi("BridgeState", { magic: Buffer.from("KPBSTAT1"), version: 2, state: 3,
  ...
  return { status: "PASS", scope: "DEVNET_TEST_ONLY_ZERO_SUPPLY_ENROLLMENT", sourceSha: manifest.sourceSha,
    ...
    feePayerBalanceLamports: String(balance.value), borshSchemaVersion: 3, bridgeAccountLayoutVersion: 2,
