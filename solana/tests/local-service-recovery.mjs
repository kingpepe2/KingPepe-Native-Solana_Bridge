...
export async function restoreClosedTestSnapshot({ context: c, contextFile, repoRoot, runRoot, sourceSha, boundary }) {
  ...
  check(/^[a-f0-9]{40}$/.test(sourceSha) && ["PENDING_NATIVE_SWEEP", "FINALIZED_MINT_PENDING_ACCOUNTING"].includes(boundary));
  ...
  try {
    ...
    expected = { deposits: original.serviceDeposits().map(v => v.operationId) };
    check(expected.deposits.length === 1);
