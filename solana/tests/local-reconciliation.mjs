...
export async function runLocalReconciliation(repoRoot, sourceSha) {
  ...
  const claims = await runLocalProtectedClaimObservation(repoRoot, async f => {
    ...
    try {
      ...
      assert.equal(result.accounting.authorizedUnmintedCredits, "0"); assert.equal(result.accounting.reserveReleaseAuthorized, false);
      ...
      // policy for an unregistered INTERNAL reserve move. Reconciliation
  ...
  return { protocol: "KINGPEPE_LOCAL_RECONCILIATION_V1", claimChecks: claims.pass, pass: passed.length, fail: 0, passed,
    state: "COMPLETED", productionReady: false, mainnetActivation: "DISABLED",
