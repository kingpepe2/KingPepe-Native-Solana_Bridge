...
//! protection and forward issuance only. Mainnet enrollment
//! starts paused; explicit enrollment-authority control is separate from minting.
...
pub const BRIDGE_INSTRUCTION_SET_MAINNET_MODE: u8 = 4;
...
pub const BRIDGE_CONFIG_INSTRUCTION_LENGTH: usize = 206;
...
pub const BRIDGE_STATE_ACCOUNT_LENGTH: usize = 8 + 1 + 1 + BRIDGE_CONFIG_STATE_LENGTH + 16;
...
pub fn decode_bridge_instruction(data: &[u8]) -> Result<BridgeInstruction, EntrypointError> {
    ...
    let expected = match tag {
        ...
        BRIDGE_INSTRUCTION_SET_MAINNET_MODE => 2,
    ...
    match tag {
        BRIDGE_INSTRUCTION_INITIALIZE => {
            ...
            Ok(BridgeInstruction::Initialize(Box::new(
                bridge_config_from_wire(wire::BridgeConfigWire {
        ...
        BRIDGE_INSTRUCTION_SET_MAINNET_MODE => {
            let value: wire::MainnetModeWire =
                borsh::from_slice(data).map_err(|_| EntrypointError::InvalidInstructionEncoding)?;
            let mode = match value.mode {
                0 => MainnetMode::Paused,
                1 => MainnetMode::Controlled,
                2 => MainnetMode::Active,
                _ => return Err(EntrypointError::InvalidInstructionEncoding),
            };
            Ok(BridgeInstruction::SetMainnetMode(mode))
        }
...
pub enum BridgeInstruction {
    Initialize(Box<BridgeConfig>),
    AcceptDepositClaim(Box<CanonicalBridgeMessage>),
    SetMainnetMode(MainnetMode),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
pub enum MainnetMode {
    Paused = 0,
    Controlled = 1,
    Active = 2,
}
...
impl BridgeInstruction {
    pub fn encode(&self) -> Result<Vec<u8>, EntrypointError> {
        let encoded = match self {
            ...
            Self::SetMainnetMode(mode) => borsh::to_vec(&wire::MainnetModeWire {
                tag: BRIDGE_INSTRUCTION_SET_MAINNET_MODE,
                mode: *mode as u8,
            }),
        };
...
pub struct BridgeStateAccount {
    ...
    /// Cumulative bridge-issued units, not live SPL supply.
...
impl BridgeProgram {
    ...
    pub fn initialize(&mut self, config: BridgeConfig) -> Result<(), BridgeError> {
        ...
        if config.environment == BridgeEnvironment::Mainnet
            && (config.mainnet_activation_enabled || !config.deposits_paused)
        {
    ...
    pub fn is_operational(&self) -> bool {
        matches!(
            self.state,
            ProgramState::LocalnetTesting
                | ProgramState::DevnetTesting
                | ProgramState::MainnetControlled
                | ProgramState::MainnetActive
        )
    }

    // In-memory model only. The on-chain entry point independently requires
    // the exact Mint enrollment account to sign, with immutable PDA binding.
    pub fn set_mainnet_mode(&mut self, mode: MainnetMode) -> Result<(), BridgeError> {
        let config = self.config.as_mut().ok_or(BridgeError::Uninitialized)?;
        set_mainnet_mode(&mut self.state, config, mode)
    }
    ...
    fn require_ready_for_deposits(&self) -> Result<&BridgeConfig, BridgeError> {
        ...
        require_ready_for_deposits(&self.state, config)?;
        Ok(config)
    }
}
...
fn require_ready_for_deposits(
    ...
    let mainnet_environment_matches = config.environment == BridgeEnvironment::Mainnet
        && matches!(
            (state, config.mainnet_activation_enabled),
            (ProgramState::MainnetControlled, false) | (ProgramState::MainnetActive, true)
        );
    if !(test_environment_matches && !config.mainnet_activation_enabled
        || mainnet_environment_matches)
    {
...
fn set_mainnet_mode(
    ...
    validate_config(config)?;
    validate_state_environment(state, config)?;
    if config.environment != BridgeEnvironment::Mainnet
        || !matches!(
            state,
            ProgramState::Phase0Disabled
                | ProgramState::MainnetControlled
                | ProgramState::MainnetActive
        )
    {
        return Err(BridgeError::InvalidMainnetTransition);
    }
    if mode == MainnetMode::Paused {
    ...
    match mode {
        MainnetMode::Controlled if !config.mainnet_activation_enabled => {
            *state = ProgramState::MainnetControlled;
        }
        MainnetMode::Active
            if matches!(
                state,
                ProgramState::MainnetControlled | ProgramState::MainnetActive
            ) =>
        {
            *state = ProgramState::MainnetActive;
            config.mainnet_activation_enabled = true;
        }
        _ => return Err(BridgeError::InvalidMainnetTransition),
    }
    config.deposits_paused = false;
    Ok(())
}

fn validate_state_environment(
    state: &ProgramState,
    config: &BridgeConfig,
) -> Result<(), BridgeError> {
    if config.environment == BridgeEnvironment::Mainnet {
        let valid = match state {
            ...
            ProgramState::MainnetControlled => !config.mainnet_activation_enabled,
            ProgramState::MainnetActive => config.mainnet_activation_enabled,
            _ => false,
        };
        if !valid {
            return Err(BridgeError::InvalidConfig);
...
fn process_instruction_accounts_with_token_cpi(
    ...
    match decode_bridge_instruction(instruction_data)? {
        ...
        BridgeInstruction::SetMainnetMode(mode) => {
            process_set_mainnet_mode_accounts(program_id, accounts, mode)
        }
    }
}

fn process_set_mainnet_mode_accounts(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    mode: MainnetMode,
) -> Result<(), EntrypointError> {
    let mut iter = accounts.iter();
    let bridge_state = next_required_account(&mut iter)?;
    let enrollment_authority = next_required_account(&mut iter)?;
    if iter.next().is_some() {
        return Err(EntrypointError::UnexpectedAdditionalAccounts);
    }
    ...
    require_account_key(
        bridge_state,
        &derive_bridge_state_pda(
            &state.config.manager_program_id,
            &state.config.mint_binding.mint,
        ),
    )?;
    require_account_key(enrollment_authority, &state.config.mint_binding.mint)?;
    require_signer(enrollment_authority)?;
    require_account_owner(enrollment_authority, &spl_token::id())?;
    // No token CPI, arbitrary payload, recipient, amount or authority change.
    // This only changes processing mode under the existing enrollment identity.
    set_mainnet_mode(&mut state.state, &mut state.config, mode)?;
...
fn process_accept_deposit_claim_accounts(
    ...
    let mut state = read_bridge_state_account(bridge_state)?;
    require_ready_for_deposits(&state.state, &state.config)?;
...
fn validate_config(config: &BridgeConfig) -> Result<(), BridgeError> {
    ...
    if config.environment == BridgeEnvironment::Mainnet
        && (bridge_messages::mainnet_deployment_identity(
            &config.manager_program_id,
            &config.transceiver_program_id,
            &config.mint_binding.mint,
        ) != Some(config.solana_deployment)
            || config.mint_binding.decimals != 8
            || config.mint_binding.native_decimals != 8)
    {
        return Err(BridgeError::InvalidConfig);
    }
    let mint = &config.mint_binding;
...
pub fn encode_bridge_state_account(
    ...
    validate_state_environment(&state.state, &state.config)?;
...
pub fn decode_bridge_state_account(data: &[u8]) -> Result<BridgeStateAccount, AccountCodecError> {
    ...
    validate_config(&config)?;
    let state = program_state_from_u8(value.state)?;
    validate_state_environment(&state, &config)?;
...
fn program_state_to_u8(state: &ProgramState) -> u8 {
    match state {
        ...
        ProgramState::MainnetControlled => 4,
        ProgramState::MainnetActive => 5,
    }
}
...
fn program_state_from_u8(value: u8) -> Result<ProgramState, AccountCodecError> {
    match value {
        ...
        4 => Ok(ProgramState::MainnetControlled),
        5 => Ok(ProgramState::MainnetActive),
...
pub enum BridgeError {
    ...
    #[error("Mainnet processing-mode transition is invalid")]
    InvalidMainnetTransition,
...
mod tests {
    ...
    fn mainnet_config() -> BridgeConfig {
        let mut config = account_execution_config();
        config.environment = BridgeEnvironment::Mainnet;
    ...
    fn deployment(config: &BridgeConfig) -> DeploymentIdentity {
        DeploymentIdentity {
            protocol_id: 1,
            native_network: if config.environment == BridgeEnvironment::Mainnet {
                bridge_messages::NATIVE_MAINNET_DOMAIN
            } else {
                2
            },
            native_genesis: if config.environment == BridgeEnvironment::Mainnet {
                bridge_messages::NATIVE_MAINNET_GENESIS
            } else {
                h(8)
            },
            solana_deployment: config.solana_deployment,
    ...
    fn transceiver(config: &BridgeConfig) -> TransceiverProgram {
        TransceiverProgram::initialize(TransceiverConfig {
            ...
            protocol_id: 1,
            native_network: deployment(config).native_network,
            native_genesis: deployment(config).native_genesis,
            active: true,
            key_epoch: config.key_epoch,
        })
        .unwrap()
    ...
    fn compact_initialization_rejects_premine_freeze_and_old_serialization() {
        ...
        for frozen in [false, true] {
            ...
            assert_eq!(
                BridgeInstruction::Initialize(Box::new(invalid)).encode(),
    ...
    fn bridge_instruction_abi_round_trips_and_rejects_trailing_data() {
        let config = base_config();
        let initialize = BridgeInstruction::Initialize(Box::new(config.clone()));
        ...
        // No instruction is assigned to the retired redemption tag.
        for bytes in [vec![3], vec![3; 1 + bridge_messages::MESSAGE_LENGTH]] {
            assert_eq!(
                decode_bridge_instruction(&bytes),
                Err(EntrypointError::UnsupportedInstructionTag(3))
            );
        }
    ...
    #[test]
    fn issued_counter_rejects_unaccounted_live_supply_before_mint_cpi() {
    ...
    fn economic_action_gates_reject_pause_hard_stop_and_environment_bypass() {
        ...
        {
            assert_eq!(require_ready_for_deposits(&ready, &config), Ok(()));
            assert!(require_ready_for_deposits(&ProgramState::Phase0Disabled, &config).is_err());
            for environment in [BridgeEnvironment::Devnet, BridgeEnvironment::Mainnet] {
                let mut disabled = config.clone();
                disabled.environment = environment;
                assert!(require_ready_for_deposits(&ready, &disabled).is_err());
            }
            let mut stopped = config.clone();
            stopped.hard_stop = true;
            assert!(require_ready_for_deposits(&ready, &stopped).is_err());
            let mut paused = config.clone();
            paused.deposits_paused = true;
            assert!(require_ready_for_deposits(&ready, &paused).is_err());
        }
    }
    ...
    fn initialization_rejects_reinitialization_and_keeps_mainnet_disabled() {
        ...
        let disabled_mainnet = self::mainnet_config();
        let mut mainnet_program = BridgeProgram::new();
        mainnet_program
    ...
    #[test]
    fn mainnet_starts_paused_and_controlled_mode_keeps_economic_checks() {
        let config = mainnet_config();
        let mut bridge = BridgeProgram::new();
        bridge.initialize(config.clone()).unwrap();
        assert!(!bridge.is_operational());
        assert_eq!(
            bridge.set_mainnet_mode(MainnetMode::Active),
            Err(BridgeError::InvalidMainnetTransition)
        );
        bridge.set_mainnet_mode(MainnetMode::Controlled).unwrap();
        assert!(!bridge.config().unwrap().mainnet_activation_enabled);
        let mut receiver = transceiver(&config);
        let deposit = deposit_message(&config);
        let context = accounts(&config);
        assert_eq!(
            bridge.accept_deposit_claim(&mut receiver, &deposit, &context),
            Err(BridgeError::MissingReceipt)
        );
        receiver
            .verify_message(
                &deposit,
                &observations(&config, deposit.message_digest().unwrap()),
            )
            .unwrap();
        bridge
            .accept_deposit_claim(&mut receiver, &deposit, &context)
            .unwrap();
        assert_eq!(bridge.minted_supply(), 5000);
        assert_eq!(
            bridge.accept_deposit_claim(&mut receiver, &deposit, &context),
            Err(BridgeError::Replay)
        );
        bridge.set_mainnet_mode(MainnetMode::Paused).unwrap();
        assert_eq!(
            bridge.accept_deposit_claim(&mut receiver, &deposit, &context),
        ...
        bridge.set_mainnet_mode(MainnetMode::Controlled).unwrap();
        assert_eq!(bridge.minted_supply(), 5000);
        bridge.set_mainnet_mode(MainnetMode::Active).unwrap();
        assert!(bridge.config().unwrap().mainnet_activation_enabled);
        assert_eq!(
            bridge.set_mainnet_mode(MainnetMode::Controlled),
            Err(BridgeError::InvalidMainnetTransition)
        );
        bridge.set_mainnet_mode(MainnetMode::Paused).unwrap();
        assert_eq!(
    ...
    #[test]
    fn mainnet_rejects_mixed_domains_decimals_and_unpaused_enrollment() {
        let config = mainnet_config();
        let contradictory = BridgeStateAccount {
            state: ProgramState::MainnetActive,
            config: config.clone(),
            minted_supply: 0,
        };
        assert!(encode_bridge_state_account(&contradictory).is_err());
        let mut stopped_config = config.clone();
        stopped_config.hard_stop = true;
        let mut stopped = BridgeProgram::new();
        stopped.initialize(stopped_config).unwrap();
        assert_eq!(
            stopped.set_mainnet_mode(MainnetMode::Controlled),
            Err(BridgeError::PausedOrHardStopped)
        );
        let mut bad = config.clone();
        bad.deposits_paused = false;
        assert_eq!(
            BridgeProgram::new().initialize(bad),
            Err(BridgeError::MainnetActivationDisabled)
        );
        let mut bad = config.clone();
        bad.solana_deployment = h(99);
        assert_eq!(
            BridgeProgram::new().initialize(bad),
            Err(BridgeError::InvalidConfig)
        );
        let mut bad = config.clone();
        bad.mint_binding.decimals = 9;
        bad.mint_binding.native_decimals = 9;
        assert_eq!(
            BridgeProgram::new().initialize(bad),
            Err(BridgeError::InvalidConfig)
        );
        let mut test_program = BridgeProgram::new();
        test_program.initialize(base_config()).unwrap();
        assert_eq!(
            test_program.set_mainnet_mode(MainnetMode::Controlled),
            Err(BridgeError::InvalidMainnetTransition)
        );
        let mut bridge = BridgeProgram::new();
        bridge.initialize(config.clone()).unwrap();
        ...
        wrong.environment = BridgeEnvironment::Devnet;
        let wrong_message = deposit_message(&wrong);
        assert_eq!(
            bridge.accept_deposit_claim(
                &mut transceiver(&config),
                &wrong_message,
                &accounts(&config)
            ),
            Err(BridgeError::MessageDomainMismatch)
        );
    }

    #[test]
    fn mainnet_mode_entrypoint_requires_exact_enrollment_signature_and_never_mints() {
        let config = mainnet_config();
        let original = BridgeStateAccount {
            state: ProgramState::Phase0Disabled,
            config: config.clone(),
            minted_supply: 0,
        };
        let bytes = encode_bridge_state_account(&original).unwrap();
        let state_key = Pubkey::new_from_array(derive_bridge_state_pda(
            &config.manager_program_id,
            &config.mint_binding.mint,
        ));
        let state = test_account(state_key, id(), bytes.clone(), false, true);
        let mut mint = test_account(
            Pubkey::new_from_array(config.mint_binding.mint),
            spl_token::id(),
            mint_account_data(&config, 0),
            false,
        ...
        let control = BridgeInstruction::SetMainnetMode(MainnetMode::Controlled)
            .encode()
            .unwrap();
        assert_eq!(control, vec![4, 1]);
        assert_eq!(
            process_instruction_accounts(&id(), &[state.clone(), mint.clone()], &control),
            Err(EntrypointError::MissingRequiredSignature)
        );
        ...
        assert_eq!(
            process_instruction_accounts(&id(), &[state.clone(), other], &control),
            Err(EntrypointError::AccountKeyMismatch)
        );
        mint.is_signer = true;
        let wrong_state = test_account(Pubkey::new_unique(), id(), bytes.clone(), false, true);
        assert_eq!(
            process_instruction_accounts(&id(), &[wrong_state, mint.clone()], &control),
            Err(EntrypointError::AccountKeyMismatch)
        );
        let wrong_owner = test_account(state_key, Pubkey::new_unique(), bytes.clone(), false, true);
        assert_eq!(
            process_instruction_accounts(&id(), &[wrong_owner, mint.clone()], &control),
            Err(EntrypointError::AccountOwnerMismatch)
        );
        let readonly = test_account(state_key, id(), bytes.clone(), false, false);
        assert_eq!(
            process_instruction_accounts(&id(), &[readonly, mint.clone()], &control),
            Err(EntrypointError::AccountNotWritable)
        );
        let activate = BridgeInstruction::SetMainnetMode(MainnetMode::Active)
            .encode()
            .unwrap();
        assert_eq!(
            process_instruction_accounts(&id(), &[state.clone(), mint.clone()], &activate),
            Err(BridgeError::InvalidMainnetTransition.into())
        );
        assert_eq!(state.data.borrow().to_vec(), bytes);
        let mint_before = mint.data.borrow().to_vec();
        process_instruction_accounts(&id(), &[state.clone(), mint.clone()], &control).unwrap();
        let after = read_bridge_state_account(&state).unwrap();
        assert_eq!(after.state, ProgramState::MainnetControlled);
        assert!(!after.config.mainnet_activation_enabled);
        assert_eq!(after.minted_supply, 0);
        process_instruction_accounts(&id(), &[state.clone(), mint.clone()], &activate).unwrap();
        assert_eq!(
            read_bridge_state_account(&state).unwrap().state,
            ProgramState::MainnetActive
        );
        let pause = BridgeInstruction::SetMainnetMode(MainnetMode::Paused)
            .encode()
            .unwrap();
        process_instruction_accounts(&id(), &[state.clone(), mint.clone()], &pause).unwrap();
        let after = read_bridge_state_account(&state).unwrap();
        assert!(after.config.deposits_paused);
        assert_eq!(mint.data.borrow().to_vec(), mint_before);
        for malformed in [vec![4], vec![4, 3], vec![4, 0, 0], vec![4, 255]] {
            assert!(decode_bridge_instruction(&malformed).is_err());
        }
    }
    ...
    fn devnet_test_state_preserves_borsh_replay_accounting_and_pause() {
        ...
        {
            assert_eq!(require_ready_for_deposits(bridge.state(), &config), Ok(()));
            for wrong in [BridgeEnvironment::Localnet, BridgeEnvironment::Mainnet] {
                let mut other = config.clone();
                other.environment = wrong;
                assert_eq!(
                    require_ready_for_deposits(bridge.state(), &other),
                    Err(BridgeError::ActivationDisabled)
                );
            }
            let mut paused = config.clone();
            paused.deposits_paused = true;
            assert_eq!(
                require_ready_for_deposits(bridge.state(), &paused),
                Err(BridgeError::PausedOrHardStopped)
            );
            paused = config.clone();
