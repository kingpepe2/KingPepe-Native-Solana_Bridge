...
//! forward instruction tags and reject incompatible account layouts. They are
...
mod tests {
    ...
    #[test]
    fn mainnet_control_borsh_bytes_match_javascript_vectors() {
        for (name, mode) in [
            ("MainnetPause", 0),
            ("MainnetControlled", 1),
            ("MainnetActive", 2),
        ] {
            check(name, MainnetModeWire { tag: 4, mode });
        }
        assert!(borsh::from_slice::<MainnetModeWire>(&[4]).is_err());
        assert!(borsh::from_slice::<MainnetModeWire>(&[4, 1, 0]).is_err());
    }
}
