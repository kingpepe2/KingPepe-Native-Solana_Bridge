...
export function createWithdrawalInstruction({ encodedMessageHex, userAuthority, payer, sourceTokenAccount }) {
  ...
  return { program: manager, accounts: [meta(pda("kingpepe-bridge-state", m.deployment.mint), true),
    ...
    meta(payer, true, true), meta(SYSTEM)], data: encodeBridgeAbi("RecordWithdrawal", {
      tag: 3, message: m.encoded, burn: { tokenProgramId: base58Decode(TOKEN), mint: m.deployment.mint,
        authority: base58Decode(userAuthority), amountAtomic: m.amountAtomic, decimals: 8 },
    }) };
}
