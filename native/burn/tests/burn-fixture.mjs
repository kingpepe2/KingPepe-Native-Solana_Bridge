...
export function burnFixture() {
  const {manifest,snapshot}=deploymentFixture(),root=schnorr.utils.randomSecretKey(),payer=ed25519.utils.randomSecretKey(),wallet=ed25519.utils.randomSecretKey(),attesters=[ed25519.utils.randomSecretKey(),ed25519.utils.randomSecretKey()];
  const pub=key=>hex(ed25519.getPublicKey(key));
  Object.assign(manifest.config,{protocolId:1,nativeNetwork:8000111,attesters:attesters.map(a=>base58Encode(ed25519.getPublicKey(a)))});
  const tc=decodeBridgeAbi('TransceiverState',Buffer.from(snapshot.accounts[4].data[0],'base64'));
  Object.assign(tc.config,{protocolId:1,nativeNetwork:8000111,authorizedAttesters:attesters.map(seed=>ed25519.getPublicKey(seed))});
  snapshot.accounts[4].data[0]=encodeBridgeAbi('TransceiverState',tc).toString('base64');
  const binding={protocolId:1,nativeNetwork:8000111,nativeGenesis:manifest.nativeGenesisHex,solanaGenesis:hex(base58Decode(manifest.solanaGenesis)),solanaDeployment:manifest.solanaDeploymentHex,
  ...
  const {destination:_d,nonce:_n,...deployment}=binding,context={environment:'localnet',deployment,keyEpoch:1,policyEpoch:1,attesters:attesters.map(pub)};
  ...
  const state=initialBurnJournal(binding,{context,feePayerHex:pub(payer)});state.paused=false;state.pauseReason='TEST_REVIEWED';
  const op=issueBurnDeposit(state,binding,20),id=op.operationId,fees=burnOperationalDestination(binding);
  const deposit={txid:hash('deposit'),vout:0,amountAtomic:'100000',blockHash:hash('deposit-block'),height:21,confirmations:12};
  const plan=planNativeBurn({operationId:id,inputs:[{txid:deposit.txid,vout:0,amountAtomic:deposit.amountAtomic,scriptPubKeyHex:burnDepositDestination(binding).scriptPubKeyHex},
  ...
  return {manifest,snapshot,policy,context,binding,state,id,plan,evidence,deposit,payer,root,finalize,authorize,
