...
import { NATIVE_MAINNET_GENESIS, NATIVE_MAINNET_DOMAIN, SOLANA_MAINNET_GENESIS, assertMainnetProtectedDeployment } from "../../../shared/network-identity.mjs";
...
const INTENT_FIELDS = Object.freeze(["protocol", "mode", "purpose", "nativeNetwork", "nativeGenesisHash",
  ...
  "operationId", "proofFingerprint", "unsignedNativeTransactionId", "transactionCommitment",
...
export function createNativeSigningPolicy(options) {
  ...
  return signingPolicy(options, localPolicyContext(options));
}

// Configuration admission only. A signing policy cannot replace independent
// Native evidence, protected A+B state, the journal or a live supervisor grant.
export function createMainnetNativeSigningPolicy(options) {
  options = dataRecord(options, "Mainnet signing policy");
  if (options.environment !== "mainnet" || options.nativeNetwork !== "mainnet" ||
      !["CONTROLLED", "ACTIVE"].includes(options.activationMode) ||
      options.productionSigningAuthorized !== true || options.productionBroadcastAuthorized !== true ||
      options.productionReady !== (options.activationMode === "ACTIVE") ||
      options.mainnetActivation !== (options.activationMode === "ACTIVE" ? "ENABLED" : "DISABLED"))
    throw new Error("MainnetSigningAdmissionRequired");
  if (options.productionLimitPolicy !== "UNBOUNDED_BY_TEAM_DECISION" || options.maxAmountAtomic !== undefined || options.maxFeeAtomic !== undefined)
    throw new Error("MainnetSigningPolicyConfigurationRejected");
  assertMainnetProtectedDeployment({ environment: options.environment, nativeGenesis: options.nativeGenesisHash,
    solanaDeployment: options.solanaDeployment, keyEpoch: options.keyEpoch }, {
    environment: options.environment, protocolId: 1, nativeNetwork: NATIVE_MAINNET_DOMAIN,
    nativeGenesis: options.nativeGenesisHash, solanaGenesis: options.solanaGenesis, solanaDeployment: options.solanaDeployment,
    managerProgramId: options.bridgeProgramId, transceiverProgramId: options.transceiverProgramId, mint: options.mint, keyEpoch: options.keyEpoch });
  const feePolicy = validateNativeFeePolicy(dataRecord(options.nativeFeePolicy, "Mainnet Native fee policy"));
  let controlledOperationIds = [];
  if (options.activationMode === "CONTROLLED") {
    controlledOperationIds = dataArray(options.controlledOperationIds, 2, "controlled operations").map(id => assertHashHex(id, "controlled operation"));
    if (controlledOperationIds.length < 1 || new Set(controlledOperationIds).size !== controlledOperationIds.length)
      throw new Error("MainnetControlledOperationsRequired");
  } else if (options.controlledOperationIds !== undefined) throw new Error("MainnetActivePolicyControlListRejected");
  return signingPolicy(options, Object.freeze({ environment: "mainnet", nativeNetwork: "mainnet", nativeGenesisHash: NATIVE_MAINNET_GENESIS,
    solanaGenesis: SOLANA_MAINNET_GENESIS, productionReady: options.productionReady, productionSigningAuthorized: true,
    productionBroadcastAuthorized: true, mainnetActivation: options.mainnetActivation, activationMode: options.activationMode,
    productionLimitPolicy: options.productionLimitPolicy, nativeFeePolicy: feePolicy, controlledOperationIds: Object.freeze(controlledOperationIds) }));
}

function signingPolicy(options, context) {
  ...
  for (const operation of dataArray(options.authorizedOperations, MAX_POLICY_OPERATIONS, "authorized operations")) {
    ...
    if (context.environment === "mainnet" && (normalized.purpose !== "RESERVE_SWEEP" ||
        normalized.nativeNetwork !== context.nativeNetwork || normalized.nativeGenesisHash !== context.nativeGenesisHash ||
        normalized.solanaDeployment !== options.solanaDeployment || normalized.bridgeProgramId !== options.bridgeProgramId ||
        normalized.transceiverProgramId !== options.transceiverProgramId || normalized.mint !== options.mint || normalized.keyEpoch !== options.keyEpoch ||
        context.activationMode === "CONTROLLED" && !context.controlledOperationIds.includes(normalized.operationId)))
      throw new Error("MainnetOperationNotAdmitted");
  ...
  const policy = Object.freeze({
    ...
    ...(context.environment === "mainnet" ? {} : { maxAmountAtomic: BigInt(canonicalUintDecimal(options.maxAmountAtomic, "policy max amount")) }),
    maxFeeAtomic: context.environment === "mainnet" ? BigInt(context.nativeFeePolicy.maximumFeeAtomic) : BigInt(canonicalUintDecimal(options.maxFeeAtomic, "policy max fee")),
...
// Offline production key preparation only. This capability never appears in
// authorizationSnapshots, so it cannot approve an economic signing intent.
export function createMainnetNativeDkgPolicy(options) {
  options = dataRecord(options, "Mainnet DKG policy");
  if (options.environment !== "mainnet" || options.nativeNetwork !== "mainnet" ||
      options.nativeGenesisHash !== NATIVE_MAINNET_GENESIS || options.solanaGenesis !== SOLANA_MAINNET_GENESIS)
    throw new Error("MainnetDkgNetworkBindingRequired");
  for (const flag of ["productionReady", "productionSigningAuthorized", "productionBroadcastAuthorized"])
    if (options[flag] !== false) throw new Error("MainnetDkgPreparationOnly");
  if (options.mainnetActivation !== "DISABLED") throw new Error("MainnetDkgPreparationOnly");
  const ids = [options.bridgeProgramId, options.transceiverProgramId, options.mint];
  ids.forEach(id => { assertHashHex(id, "Mainnet DKG public identity"); if (/^0+$/u.test(id)) throw new Error("MainnetDkgPublicIdentityRequired"); });
  if (new Set(ids).size !== ids.length) throw new Error("MainnetDkgDistinctIdentitiesRequired");
  const policy = Object.freeze({ environment: "mainnet", nativeNetwork: "mainnet", nativeGenesisHash: NATIVE_MAINNET_GENESIS,
    productionReady: false, productionSigningAuthorized: false, productionBroadcastAuthorized: false, mainnetActivation: "DISABLED",
    purpose: "MAINNET_DKG_PREPARATION", solanaDeployment: assertHashHex(options.solanaDeployment, "Mainnet DKG deployment"),
    bridgeProgramId: options.bridgeProgramId, transceiverProgramId: options.transceiverProgramId, mint: options.mint,
    keyEpoch: checkedSafeEpoch(options.keyEpoch, "Mainnet DKG epoch") });
  localDkgPolicies.add(policy);
  return policy;
}
...
export function validateNativeSigningIntent(intent) {
  ...
  if (intent.purpose !== "RESERVE_SWEEP") {
  ...
  const normalized = {
    ...
    pauseDeposits: checkedBoolean(intent.pauseDeposits, "pauseDeposits"),
...
export function evaluateNativeSigningPolicy(policy, intent) {
  ...
  const checks = {
    ...
    depositsOpen: !normalized.pauseDeposits,
    ...
    amountWithinPolicy: amount > 0n && (policy.environment === "mainnet" ? policy.productionLimitPolicy === "UNBOUNDED_BY_TEAM_DECISION" : amount <= policy.maxAmountAtomic),
    feeWithinPolicy: fee <= policy.maxFeeAtomic,
    reserveDestinationExact: normalized.recipientScriptPubKeyHex === policy.reserveScriptPubKeyHex,
