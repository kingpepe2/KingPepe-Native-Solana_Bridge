...
import { NATIVE_MAINNET_GENESIS, SOLANA_MAINNET_GENESIS, mainnetDeploymentIdentity } from "../../../shared/network-identity.mjs";
import { base58Encode } from "../../../services/bridge-validator/solana-deposit-claim-transaction-plan.mjs";
import { assertNativeSigningPolicy } from "../policy/native-signing-policy.mjs";
import { nativeFrostKeyContext } from "../policy/dkg-request.mjs";

test("Mainnet DKG preparation binds the production domain but grants no economic signing or plaintext fallback", () => {
  const options = { environment: "mainnet", nativeNetwork: "mainnet", nativeGenesisHash: NATIVE_MAINNET_GENESIS,
    solanaGenesis: SOLANA_MAINNET_GENESIS, solanaDeployment: h("prepared-mainnet-deployment"), bridgeProgramId: h("mainnet-bridge"),
    transceiverProgramId: h("mainnet-transceiver"), mint: h("mainnet-mint"), keyEpoch: 1,
    productionReady: false, productionSigningAuthorized: false, productionBroadcastAuthorized: false, mainnetActivation: "DISABLED" };
  const policy = createMainnetNativeDkgPolicy(options), context = nativeFrostKeyContext(policy);
  assert.equal(context.environment, "mainnet");
  assert.equal(createTwoPartyDkgRequest({ epoch: 1, context }).threshold, 2);
  assert.throws(() => assertNativeSigningPolicy(policy), /SnapshotRequired/);
  assert.throws(() => createNativeSigningPolicy({ ...options, authorizedOperations: [] }), /LocalnetOnly/);
  assert.throws(() => FileBackedFrostStateStore.createLocal({ policy }), /PlaintextLocalOnly/);
  assert.throws(() => new NativeFrostSigner({ policy, signerId: "KINGPEPE_FROST_A", index: 0, stateStore: {} }), /MainnetFrostProtectedStateRequired/);
  for (const mutation of [{ nativeGenesisHash: REGTEST_GENESIS }, { solanaGenesis: "devnet" }, { nativeNetwork: "regtest" },
    { environment: "localnet" }, { productionSigningAuthorized: true }, { productionBroadcastAuthorized: true },
    { productionReady: true }, { mainnetActivation: "ENABLED" }, { mint: options.bridgeProgramId }])
    assert.throws(() => createMainnetNativeDkgPolicy({ ...options, ...mutation }));
});
...
function baseAuthorization(overrides = {}) {
  ...
  return {
    ...
    mint: h("kpepe-mint"),
    keyEpoch: 1,
    proofFingerprint: h("finalized-deposit-proof"),
    ...
    signingInputIndex: 0,
    recipientScriptPubKeyHex: p2tr("reserve"),
...
function mainnetPolicyFixture(activationMode = "CONTROLLED") {
  const identities = { bridgeProgramId: h("isolated-mainnet-signing-bridge"), transceiverProgramId: h("isolated-mainnet-signing-transceiver"), mint: h("isolated-mainnet-signing-mint") };
  const pub = key => base58Encode(Buffer.from(key, "hex"));
  const solanaDeployment = mainnetDeploymentIdentity({ manager: pub(identities.bridgeProgramId), transceiver: pub(identities.transceiverProgramId), mint: pub(identities.mint) });
  const intent = baseAuthorization({ ...identities, nativeNetwork: "mainnet", nativeGenesisHash: NATIVE_MAINNET_GENESIS, solanaDeployment });
  const options = { environment: "mainnet", nativeNetwork: "mainnet", nativeGenesisHash: NATIVE_MAINNET_GENESIS,
    solanaGenesis: SOLANA_MAINNET_GENESIS, solanaDeployment, ...identities, keyEpoch: 1,
    activationMode, productionReady: activationMode === "ACTIVE", mainnetActivation: activationMode === "ACTIVE" ? "ENABLED" : "DISABLED",
    productionSigningAuthorized: true, productionBroadcastAuthorized: true, productionLimitPolicy: "UNBOUNDED_BY_TEAM_DECISION",
    // TEST fixture fee ceilings only. No production fee value is selected here.
    nativeFeePolicy: { policy: "DYNAMIC_NODE_ESTIMATE_WITH_CAP", minimumRelayAtomicPerKvB: "100", maximumAtomicPerKvB: "1000", maximumFeeAtomic: "100000" },
    reserveScriptPubKeyHex: intent.changeScriptPubKeyHex, authorizedOperations: [intent],
    ...(activationMode === "CONTROLLED" ? { controlledOperationIds: [intent.operationId] } : {}) };
  return { intent, options };
}

test("Mainnet signing configuration requires explicit admission, complete identity and the measured-fee policy shape", () => {
  const { options, intent } = mainnetPolicyFixture();
  const policy = createMainnetNativeSigningPolicy(options);
  assert.equal(evaluateNativeSigningPolicy(policy, intent).result, "APPROVED");
  assert.equal(policy.productionLimitPolicy, "UNBOUNDED_BY_TEAM_DECISION"); assert.equal(Object.hasOwn(policy, "maxAmountAtomic"), false);
  assert.equal(nativeFrostKeyContext(policy).environment, "mainnet");
  for (const mutation of [{ environment: "localnet" }, { nativeNetwork: "regtest" }, { nativeGenesisHash: REGTEST_GENESIS },
    { solanaGenesis: "devnet" }, { mint: h("substituted-mint") }, { solanaDeployment: h("substituted-deployment") },
    { activationMode: "PAUSED" }, { productionSigningAuthorized: false }, { productionBroadcastAuthorized: false },
    { productionReady: true }, { mainnetActivation: "ENABLED" }, { productionLimitPolicy: "BOUNDED_TEST" },
    { maxAmountAtomic: "1000" }, { maxFeeAtomic: "1000" }, { nativeFeePolicy: undefined },
    { nativeFeePolicy: { ...options.nativeFeePolicy, policy: "FIXED" } }])
    assert.throws(() => createMainnetNativeSigningPolicy({ ...options, ...mutation }));
  assert.throws(() => createNativeSigningPolicy(options), /LocalnetOnly/);
  assert.throws(() => new NativeFrostSigner({ policy, signerId: "KINGPEPE_FROST_A", index: 0, stateStore: {} }), /MainnetFrostProtectedStateRequired/);
});

test("Mainnet controlled policy enrolls only exact operation identities and retains fee and immutable-intent checks", () => {
  const { options, intent } = mainnetPolicyFixture(), policy = createMainnetNativeSigningPolicy(options);
  for (const ids of [[], [h("wrong-operation")], [intent.operationId, intent.operationId], [intent.operationId, h("two"), h("three")]])
    assert.throws(() => createMainnetNativeSigningPolicy({ ...options, controlledOperationIds: ids }));
  assert.throws(() => createMainnetNativeSigningPolicy({ ...options, authorizedOperations: [{ ...intent, purpose: "RESERVE_MIGRATION" }] }), /unsupported Native FROST signing purpose/);
  for (const mutation of [{ nativeNetwork: "regtest" }, { nativeGenesisHash: REGTEST_GENESIS }, { solanaDeployment: h("wrong-deployment") },
    { bridgeProgramId: h("wrong-program") }, { transceiverProgramId: h("wrong-transceiver") }, { mint: h("wrong-mint") }, { keyEpoch: 2 }])
    assert.throws(() => createMainnetNativeSigningPolicy({ ...options, authorizedOperations: [{ ...intent, ...mutation }] }), /MainnetOperationNotAdmitted/);
  for (const mutation of [{ amountAtomic: "1" }, { recipientScriptPubKeyHex: p2tr("wrong-recipient") }, { feeAtomic: "100001" },
    { operationId: h("wrong-operation") }, { pauseDeposits: true }, { hardStop: true }])
    assert.equal(evaluateNativeSigningPolicy(policy, { ...intent, ...mutation }).result, "REJECTED");
  options.controlledOperationIds[0] = h("changed-after-enrollment"); options.authorizedOperations[0].amountAtomic = "1";
  assert.equal(evaluateNativeSigningPolicy(policy, policy.authorizedOperations[0]).result, "APPROVED");
});

test("active Mainnet unbounded policy retains u64 parsing and cannot reuse a controlled or paused configuration", () => {
  const { options, intent } = mainnetPolicyFixture("ACTIVE"), large = { ...intent, amountAtomic: "18446744073709551615" };
  const policy = createMainnetNativeSigningPolicy({ ...options, authorizedOperations: [large] });
  assert.equal(evaluateNativeSigningPolicy(policy, large).result, "APPROVED");
  assert.throws(() => createMainnetNativeSigningPolicy({ ...options, productionReady: false }));
  assert.throws(() => createMainnetNativeSigningPolicy({ ...options, mainnetActivation: "DISABLED" }));
  assert.throws(() => createMainnetNativeSigningPolicy({ ...options, controlledOperationIds: [intent.operationId] }));
  assert.throws(() => createMainnetNativeSigningPolicy({ ...options, authorizedOperations: [{ ...large, amountAtomic: "18446744073709551616" }] }));
});

test("Mainnet coordinator cannot fall back to direct participant calls", async () => {
  const { intent } = mainnetPolicyFixture(); let calls = 0;
  const coordinator = new NativeFrostCoordinator({ signers: REQUIRED_FROST_SIGNERS.map(signerId => ({ signerId, isAvailable: () => true,
    verifyNativeEvidence() { calls++; }, signingCommitment() { calls++; } })), publicPackage: {}, aggregateTweakedXOnlyPublicKey: h("unused-public-key") });
  assert.throws(() => coordinator.signAutomatically(intent), /MainnetProtectedParticipantsRequired/);
  await assert.rejects(coordinator.signAutomaticallyWithNativeEvidence(intent), /MainnetProtectedParticipantsRequired/);
...
for (const [field, value] of [["purpose", "RESERVE_MIGRATION"], ["proofFingerprint", h("substituted-proof")],
  ...
  test(`full intent enrollment rejects substitution of ${field}`, () => {
    ...
    assert.equal(evaluateNativeSigningPolicy(policy, intent).result, "APPROVED");
    if (field === "purpose") assert.throws(() => evaluateNativeSigningPolicy(policy, { ...intent, [field]: value }), /unsupported Native FROST signing purpose/);
    else assert.equal(evaluateNativeSigningPolicy(policy, { ...intent, [field]: value }).result, "REJECTED");
  });
}
  ...
  try {
    for (const signer of [runtime.signerA, runtime.signerB]) {
      for (const changes of [{ purpose: "RESERVE_MIGRATION" }, { proofFingerprint: h("other-proof") },
        ...
        const request = changes.purpose ? { ...frostRequestFor(intent), intent: { ...intent, ...changes } } : frostRequestFor({ ...intent, ...changes });
        const expected = changes.purpose ? /unsupported Native FROST signing purpose/ : /authorizedOperationExact/u;
        assert.throws(() => signer.signingCommitment(request), expected);
        assert.throws(() => signer.signatureShare(request, []), expected);
      }
    }
...
test("every normalized intent field is covered by authorization or independent policy guards", () => {
  ...
  for (const [field, value] of Object.entries(intent)) {
    ...
    else if (field === "purpose") changed = "UNSUPPORTED";
...
test("enrolled purpose and evidence metadata stay detached and immutable", () => {
  ...
  assert.throws(() => evaluateNativeSigningPolicy(policy, auth), /unsupported Native FROST signing purpose/);
});
...
test("intent stop and pause flags cannot turn missing or malformed evidence into false", () => {
  for (const flag of ["hardStop", "pauseDeposits"]) {
...
test("coordinator rejects every non-sweep signing purpose", t => {
  const x = coordinatorJournalFixture(t);
  for (const purpose of ["WITHDRAWAL", "RESERVE_MIGRATION", "UNKNOWN"]) {
    assert.throws(() => createNativeFrostSigningRequest({ ...x.request.intent, purpose }), /unsupported Native FROST signing purpose/);
  }
});
