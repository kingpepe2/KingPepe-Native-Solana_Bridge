...
export function prepareRegtestRecoveryTransaction(options) { return prepareRecoveryTransaction(options, false); }
export function prepareMainnetRecoveryTransaction(options) { return prepareRecoveryTransaction(options, true); }
function prepareRecoveryTransaction({ depositPolicy, fundingTransactionHex, outputIndex,
  amountAtomic, destinationScriptPubKeyHex, feeAtomic, maximumFeeAtomic }, mainnet) {
  const policy = (mainnet ? buildMainnetRecoverableDeposit : buildRegtestRecoverableDeposit)(depositPolicy);
  ...
  return Object.freeze({ status: "UNSIGNED_UNBROADCAST_NOT_AUTHORIZED", ...(mainnet ? { environment: "mainnet" } : { localOnly: true }), productionReady: false,
