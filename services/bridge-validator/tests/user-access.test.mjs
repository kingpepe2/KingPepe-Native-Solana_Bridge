...
test("HTTP rejects cross-origin, malformed and oversized input; no admin route or leaked errors", async t => {
  ...
  for (const route of ["sign", "mint", "resume", "pause", "withdrawals/request", "withdrawals/submit"]) assert.equal((await fetch(f.endpoint + route, { method: "POST", headers, body: "{}" })).status, 404);
  ...
  const r = await fetch(f.endpoint + "deposits/submit", { method: "POST", headers, body: "{}" });
  assert.equal(r.status, 400); assert(!(await r.text()).includes("PRIVATE_EXCEPTION"));
});
test("public balance SDK uses authenticated fixed GET routes and rejects RPC/path overrides", async t => {
  const f = await httpFixture(t);
  assert.equal((await f.client.getSolanaBalance(key(7))).amountAtomic, "9007199254740993");
  assert.equal((await f.client.getNativeBalance(witnessAddressFromScript("5120" + second))).address, witnessAddressFromScript("5120" + second));
  assert.throws(() => f.client.getSolanaBalance("../bridge/status"));
  const path = "balances/solana/" + key(7), headers = { authorization: "Bearer " + f.accessToken };
  assert.equal((await fetch(f.endpoint + path)).status, 403);
  for (const route of [path + "?rpc=x", "balances/mainnet/" + key(7), "balances/native/../pause"])
    assert.equal((await fetch(f.endpoint + route, { headers })).status, 404);
  assert.equal((await fetch(f.endpoint + path, { method: "POST", headers })).status, 404);
});
