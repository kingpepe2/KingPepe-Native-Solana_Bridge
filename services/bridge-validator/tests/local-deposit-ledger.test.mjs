...
function credit(index = 0, amount = 100n, changes = {}) {
  const message = {
    ...
    depositOutpoint: { txid: h(`backing-${index}`), vout: 0 },
...
if (process.argv[2] === "--ledger-worker") {
  ...
  test("one-way journal has no retired operation interface", t => {
    const f = fixture(t), store = f.create(), before = store.checkpoint();
    for (const name of ["enqueueConfirmedWithdrawal", "withdrawalRequests", "reserveWithdrawal", "recordWithdrawalSigned", "recordWithdrawalPaid"])
      assert.equal(store[name], undefined);
    assert.deepEqual(store.checkpoint(), before);
    assert.deepEqual(store.bridgeSnapshot(), { canonicalReserveAtomic: "0", bridgeIssuedAtomic: "0", pendingMintAtomic: "0" });
  });
