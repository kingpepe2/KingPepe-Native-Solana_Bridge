...
import { validateRegtestRecoverableDepositIntent, validateMainnetRecoverableDepositIntent } from "../../native/recovery/taproot-deposit.mjs";
import { isMainnetBinding, assertMainnetDeploymentFields } from "../../shared/network-identity.mjs";
...
export function validateDepositOperationPolicy(input) {
  ...
  const mainnet = isMainnetBinding(p);
  check(mainnet || ((p.environment === "localnet" || (p.environment === "devnet" && p.solanaGenesis === DEVNET_SOLANA_GENESIS &&
    p.nativeNetwork === 8_000_111)) && p.nativeGenesis === REGTEST_GENESIS), "DepositOperationLocalnetOnly");
  if (mainnet) {
    assertMainnetDeploymentFields(p);
    // This is the canonical wire/parser bound, not a production economic cap.
    check(p.minimumConfirmations === 12 && p.maximumAmountAtomic === "18446744073709551615" && uint(p.maximumFeeAtomic) > 0n,
      "MainnetDepositFinalityAndUnboundedPolicyRequired");
  }
...
export function validateDepositOperationPlan(input, policy) {
  ...
  hash(v.operationId); checkpoint(v.acceptedCheckpoint, p);
  ...
  const validateRecovery = p.environment === "mainnet" ? validateMainnetRecoverableDepositIntent : validateRegtestRecoverableDepositIntent;
  const depositPolicy = validateRecovery({ intent: d, policy: v.depositPolicy,
  ...
  v.signingIntents = v.signingIntents.map((inputIntent, index) => {
    ...
    for (const [field, expected] of Object.entries({ purpose: "RESERVE_SWEEP", nativeNetwork: p.environment === "mainnet" ? "mainnet" : "regtest", nativeGenesisHash: p.nativeGenesis,
      ...
      keyEpoch: p.keyEpoch, operationId: v.operationId, proofFingerprint: v.acceptedCheckpoint.evidenceDigestHex,
      ...
      reserveCommitment: e.reserveCommitment, pauseDeposits: false, hardStop: false })) check(i[field] === expected, "DepositSigningPlanSubstituted");
...
function validateFinalized(plan, fact, policy) {
  ...
  checkpoint(fact.acceptedCheckpoint, policy); hash(fact.reserveAllocationIdHex);
  ...
  check(b.genesis === policy.nativeGenesis && b.amountAtomic === plan.depositIntent.amountAtomic && b.reserveScriptHex === plan.depositPolicy.canonicalReserveScriptPubKeyHex); hash(b.chainworkHex);
  for (const value of [b.deposit, b.sweep]) {
    fields(value, ["txid", "height", "blockHash", "vout"]); hash(value.txid); hash(value.blockHash); positive(value.height, policy.environment === "mainnet" ? 1_000_000 : 4096);
