...
export function validateBurnContext(input) {
  ...
  check(['localnet','devnet'].includes(c.environment),'BurnContextTestOnly');
  validateBurnBinding({...c.deployment,destination:'01'.repeat(32),nonce:'02'.repeat(32)});
  check(Object.keys(c.deployment).length===9&&c.deployment.nativeGenesis===REGTEST_GENESIS&&
    c.deployment.nativeNetwork===8000111&&c.deployment.protocolId===1,'BurnContextNativeNetworkRejected');
