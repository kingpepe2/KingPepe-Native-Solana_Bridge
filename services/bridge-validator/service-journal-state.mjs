...
import { reduceEconomicLimitEvent } from "./economic-limits.mjs";
...
export function initialServiceState() { return { policy: null, watches: new Map(), deposits: new Map(), deliveries: new Map(), signing: null, paused: null, limits: null }; }
...
export function reduceServiceEvent(previous, event, deployment) {
  ...
  if (["LIMIT_CONFIG", "LIMIT_RESERVE", "LIMIT_SETTLE", "LIMIT_REVIEW"].includes(event.action)) {
    const result = reduceEconomicLimitEvent(previous.limits, event);
    next.limits = result.state;
    if (result.pauseReason) next.paused = result.pauseReason;
    return { state: result.unchanged ? previous : next, unchanged: result.unchanged };
  }
  if (event.action === "SIGNING") {
    ...
    check(intent.nativeNetwork === "regtest" && intent.purpose === "RESERVE_SWEEP" &&
