...
function balance(entries, mint, decimals) {
  check(Array.isArray(entries) && entries.length <= 14);
...
export function verifyDepositMintExecution(result, observation, config) {
  ...
  try {
    ...
    check(packet.length >= MESSAGE_LENGTH + 549 && packet.length <= 1232 && packet.toString("base64") === encoded &&
      packet[0] === 1 && packet.subarray(65, 69).equals(Buffer.from([1, 0, 7, 14])));
    ...
    const verified = verifySignedLocalnetSolanaDepositClaimTransaction({
      ...
      encodedMessageHex: bytes.toString("hex"),
      recentBlockhashHex: packet.subarray(517, 549).toString("hex"), lastValidBlockHeight: "0",
    ...
    check(JSON.stringify(mint.accounts) === "[3,4,8]" && mint.data === base58Encode(data) && mint.stackHeight === 2);
