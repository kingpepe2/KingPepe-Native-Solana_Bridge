...
export function compareDepositAccounting(journal, observed) {
  ...
  // Ordinary SPL burns lower circulating supply but never release backing.
  // The cumulative issued counter and explicit difference remain accounted for.
...
export async function readDepositReconciliation({ policy, manifest, operations, nativeVerifier, solanaRpc, minimumSlot }) {
  ...
  const before = await nativeVerifier.observeChain(); requireVerifiedNativeChain(before, p.nativeGenesis);
  ...
  for (const op of state.operations) {
    ...
    const receipt = await nativeVerifier.verifyReserve({ deposit: plan.inputs[0], feeInputs: plan.inputs.slice(1),
      ...
      tapscriptSpends: [plan.depositPolicy.sweep, ...plan.inputs.slice(1).map(() => undefined)], acceptedCheckpoint: credited.acceptedCheckpoint }).catch(error => {
        const spent = observedSpentNativeReserve(error, p.nativeGenesis);
        if (spent !== undefined) {
          requireVerifiedNativeChain(spent.chain, p.nativeGenesis);
    ...
    check(verifiedReserveChain(receipt, p.nativeGenesis).tipHash === before.tipHash, "ReconciliationNativeSnapshotChanged");
  ...
  const after = await nativeVerifier.observeChain(); requireVerifiedNativeChain(after, p.nativeGenesis);
