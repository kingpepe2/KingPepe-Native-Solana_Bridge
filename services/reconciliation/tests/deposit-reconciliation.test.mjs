...
const observed = { canonicalReserve: "100", mintSupplyAtomic: "100", managerMintedAtomic: "100" };

test("direct SPL burn difference never releases Native backing", () => {
  const result = compareDepositAccounting(journal, { ...observed, mintSupplyAtomic: "80" });
  assert.equal(result.coverageRequired, "80");
  assert.equal(result.unclaimedDirectBurnDifference, "20"); assert.equal(result.reserveReleaseAuthorized, false);
});
...
for (const [name, change] of [
  ...
  ["cumulative issued counter reduced", o => { o.mintSupplyAtomic = "80"; o.managerMintedAtomic = "80"; }],
